...
class MorphServiceTest(unittest.TestCase):
    def test_replace_morph_01(self):
        ...
        data_set = MOptionsDataSet(
            motion, None, None, None, False, False, morph_list, None, None
        )
    ...
    def test_replace_morph_04(self):
        ...
        self.assertTrue("B" in motion.morphs)
        self.assertEqual(
            [0, 1, 2, 3, 7, 10, 11], sorted(list(motion.morphs["B"].keys()))
        )
...
class StanceServiceTest(unittest.TestCase):
    def test_stance_shoulder_01(self):
        arm_slope = MVector3D(
            -1.837494969367981, 18.923479080200195, 0.4923856854438782
        )
        shoulder_slope = MVector3D(
            -0.8141360282897949, 19.6701602935791, 0.4931679964065552
        )
        ...
        for param in target_test_params:
            ...
            rep_shoulder_slope_cross = MVector3D.crossProduct(
                rep_shoulder_slope, rep_shoulder_slope_up
            ).normalized()
    ...
    def not_test_stance_shoulder_02(self):
        ...
        slope_test_params = [
            (x00, x01) for (x00, x01) in all_slope_test_params if x00 != x01
        ]
    ...
    def not_test_stance_shoulder_04(self):
        ...
        target_test_params = list(
            itertools.product(slope_test_params, slope_test_params)
        )
        print(len(target_test_params))
    ...
    def not_test_stance_shoulder_06(self):
        new_rep_to_pos = MVector3D(
            16.638640587237887, 19.455697325211673, 4.067013732312591
        )
        rep_base_pos = MVector3D(
            15.527995423468052, 19.158781645638516, 2.353690174209908
        )
        up_pos = MVector3D(0.02597404821369409, -0.6341368197928823, 0.7727844735774392)
        parent_qq = MQuaternion.fromEulerAngles(
            4.444080622855673, 131.6889133202979, -6.602768822699441
        )

        arm_pos = MVector3D(-1.837494969367981, 18.923479080200195, 0.4923856854438782)
        shoulder_pos = MVector3D(
            -0.8141360282897949, 19.6701602935791, 0.4931679964065552
        )
        ...
        target_test_params = list(
            itertools.product(slope_test_params, number_test_params, test_qq_param)
        )
        print(len(target_test_params))
        ...
        for params in target_test_params:
            ...
            initial = MQuaternion.fromDirection(
                rep_shoulder_slope, rep_shoulder_slope_up
            )
            ...
            from_rotation = (
                parent_qq.inverted() * from_orientation * initial.inverted() * params[2]
            )
    ...
    def not_test_stance_shoulder_07(self):
        new_rep_to_pos = MVector3D(
            16.638640587237894, 19.455697325211673, 4.067013732312591
        )
        rep_base_pos = MVector3D(
            15.541596036361701, 18.419343683301417, 2.4565491530944494
        )
        up_pos = MVector3D(
            -0.13131308583164614, -0.017769852724240698, 0.10088527183640322
        )
        parent_qq = MQuaternion.fromEulerAngles(
            4.444080622855673, 131.6889133202979, -6.602768822699441
        )
        ...
        another_arm_pos = MVector3D(
            1.837494969367981, 18.923479080200195, 0.4923856854438782
        )
        another_shoulder_pos = MVector3D(
            0.8141360282897949, 19.6701602935791, 0.493167906999588
        )
        ...
        target_test_params = list(
            itertools.product(slope_test_params, number_test_params, up_test_params)
        )
        print(len(target_test_params))
        ...
        for params in target_test_params:
            ...
            shoulder_cross = MVector3D.crossProduct(
                rep_shoulder_slope_up, rep_shoulder_up
            ).normalized()
            ...
            print(
                "original: %s"
                % MVector3D(21.338723875696445, 15.845333083479046, 37.954108757721826)
            )
    ...
    def not_test_stance_shoulder_08(self):
        ...
        rep_from_pos = MVector3D(
            16.07129122417615, 19.252113744303983, 2.952803072461259
        )
        up_pos = MVector3D(0.02597404821369409, -0.6341368197928823, 0.7727844735774392)
        parent_qq = MQuaternion.fromEulerAngles(
            4.444080622855673, 131.6889133202979, -6.602768822699441
    ...
    def test_stance_shoulder_09(self):
        ...
        for params in number_test_params:
            ...
            copy_motion = motion.copy()
            dataset = MOptionsDataSet(
                copy_motion, org_model, rep_model, "", True, False, []
            )
            dataset.test_params = params
            ...
            print(
                "right original: %s" % motion.bones["右肩"][0].rotation.toEulerAngles()
            )
            print(
                "left stance: %s"
            ...
            print(
                "left original: %s" % motion.bones["左肩"][0].rotation.toEulerAngles()
            )
