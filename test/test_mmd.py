...
class LegIkTest(unittest.TestCase):
    def test_leg_fk(self):
        ...
        for test_fno in ["0747"]:
            ...
            reader = VmdReader(
                "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Motion\\ダンス_1人\\バレリーコ 553\\ミク用バレリーコ_4870-_さとく式鶯丸ver0.90_S_20210605_084703.vmd"
            )
            ...
            for direction in ["右", "左"]:
                ...
                leg_fk_links = model.create_link_2_top_one(
                    ankle_bone_name, is_defined=False
                )
                leg_ik_links = model.create_link_2_top_one(
                    leg_ik_bone_name, is_defined=False
                )

                # デフォルト姿勢での足ボーンから見た足IKのローカル座標
                _, leg_initial_fk_matrixs = MServiceUtils.calc_global_pos(
                    model, leg_fk_links, VmdMotion(), 9, return_matrix=True
                )
                leg_initial_ik_local_pos = (
                    leg_initial_fk_matrixs[leg_fk_bone_name].inverted()
                    * model.bones[leg_ik_bone_name].position
                )
                ...
                logger.warning(
                    f"{direction}足: leg_length: {round(leg_length, 4)}, leg_ik_height: {round(leg_ik_height, 4)}"
                )
                ...
                leg_fk_local_y_axis = MVector3D.crossProduct(
                    leg_fk_local_x_axis, leg_fk_local_z_axis
                )
                logger.warning(
                    f"{direction}足: x_axis: {leg_fk_local_x_axis.to_log()}, y_axis: {leg_fk_local_y_axis.to_log()}"
                )
                ...
                # 足首
                leg_fk2ik_links.append(
                    leg_fk_links.to_links(leg_fk_bone_name).get(ankle_bone_name)
                )

                # ひざ
                knee_fk_bone = (
                    leg_fk_links.to_links(leg_fk_bone_name).get(knee_bone_name).copy()
                )
                ...
                # 足捩り
                leg_twist_bone_pos = model.bones[leg_fk_bone_name].position + (
                    (
                        model.bones[knee_bone_name].position
                        - model.bones[leg_fk_bone_name].position
                    )
                    / 2
                )
                leg_twist_fk_bone = Bone(
                    leg_twist_fk_bone_name, "", leg_twist_bone_pos, -1, 0, 0
                )
                ...
                # 足
                leg_fk_bone = (
                    leg_fk_links.to_links(leg_fk_bone_name).get(leg_fk_bone_name).copy()
                )
                ...
                for fno in motion.get_differ_fnos(
                    0,
                    [leg_fk_bone_name, leg_ik_bone_name],
                    limit_degrees=30,
                    limit_length=1,
                ):
                    ...
                    leg_fk_initial_global_3ds, leg_fk_initial_matrixs = (
                        MServiceUtils.calc_global_pos(
                            model,
                            leg_fk_links,
                            motion,
                            fno,
                            limit_links=leg_fk_links.from_links(
                                leg_fk_links.get(leg_fk_bone_name, offset=-1).name
                            ),
                            return_matrix=True,
                        )
                    )
                    leg_fk_global_3ds, leg_fk_matrixs = MServiceUtils.calc_global_pos(
                        model, leg_fk_links, motion, fno, return_matrix=True
                    )

                    leg_ik_global_3ds = MServiceUtils.calc_global_pos(
                        model, leg_ik_links, motion, fno
                    )
                    ...
                    # 足FKから見た、足IKのローカル座標
                    leg_ik_from_leg_local_pos = (
                        leg_fk_matrixs[leg_fk_bone_name].inverted() * leg_ik_global_pos
                    )
                    # 足首FKから見た、足IKのローカル座標
                    leg_ik_from_ankle_local_pos = (
                        leg_fk_matrixs[ankle_bone_name].inverted() * leg_ik_global_pos
                    )
                    ...
                    if (
                        abs(leg_ik_from_ankle_local_pos.x()) > leg_ik_height
                        or abs(leg_ik_from_ankle_local_pos.z()) > leg_ik_height
                        or leg_ik_bf.position.y() > leg_ik_height / 2
                    ) and leg_ik_from_leg_local_pos.length() > leg_ik_height * 2:
                        logger.warning(
                            f"○{fno}, {direction}足: 足首から{leg_ik_from_ankle_local_pos.to_log()}, 足から{leg_ik_from_leg_local_pos.to_log()} ({leg_ik_global_pos.to_log()})"
                        )
                        ...
                        # IK計算後のFK角度に捩り分を加算する
                        leg_twist_after_fk_bf = motion.calc_bf(
                            leg_twist_fk_bone_name, fno
                        )
                        ...
                        logger.warning(
                            f"　{fno}, {direction}足: leg_twist_after_fk_bf: {leg_twist_after_fk_bf.rotation.data()}, ({round(leg_twist_after_fk_bf.rotation.toDegreeSign(leg_fk_local_x_axis), 2)})"
                        )
                        ...
                        logger.warning(
                            f"－{fno}, {direction}足: 足首から{leg_ik_from_ankle_local_pos.to_log()}({round(leg_ik_from_ankle_local_pos.length(), 4)}), 足から{leg_ik_from_leg_local_pos.to_log()}({round(leg_ik_from_leg_local_pos.length(), 4)})"
                        )

                # # 後で削除しておく
                # del motion.bones[leg_twist_fk_bone_name]

            new_file_path = reader.file_path.replace(
                ".vmd", f"_FK_{datetime.now():%Y%m%d_%H%M%S}.vmd"
            )
...
class CameraLengthTest(unittest.TestCase):
    def test_camera_length(self):
        ...
        for n, vmd_path in enumerate(
            glob.glob(
                "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Motion\\ダンス_1人\\**\\*.vmd",
                recursive=True,
            )
        ):
            ...
            try:
                ...
                if motion.camera_cnt > 0:
                    ...
                    for fno, cdata in motion.cameras.items():
                        if cdata.length == 0:
                            logger.warning(
                                f"☆０距離カメラ: fno: {fno}, path: {vmd_path}"
                            )
                            is_zero = True
                        # else:
...
class ModelNameTest(unittest.TestCase):
    def test_model_name(self):
        ...
        for pmx_path in glob.glob(
            "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Model\\ヘタリア\\**\\*.pmx",
            recursive=True,
        ):
            reader = PmxReader(pmx_path, is_check=False)
            model = reader.read_data()
...
class MediapipeTest(unittest.TestCase):
    def test_init(self):
        ...
        reader = VmdReader(
            "E:\\MMD\\MikuMikuDance_v926x64\\Work\\202101_hand\\init\\init_avi_20210125_060727\\motion\\output_20210125_060729_no000.vmd"
        )
        motion = reader.read_data()
        ...
        for bone_name, bf_list in motion.bones.items():
            for fno, bf in bf_list.items():
                ...
                logger.warning(
                    f"{bf.name}: MQuaternion.fromEulerAngles({euler.x()}, {euler.y()}, {euler.z()})"
                )
        ...
        data_set = MOptionsDataSet(
            ...
            model,
            model,
            "E:\\MMD\\MikuMikuDance_v926x64\\Work\\202101_hand\\init\\init_avi_20210125_060727\\motion\\output_20210125_060729_no000_inverted.vmd",
        )
        writer = VmdWriter(data_set)
        writer.write()
...
class HomeHarukaTest(unittest.TestCase):
    def test_awaodori(self):
        ...
        reader = VmdReader(
            "D:\\MMD\\MikuMikuDance_v926x64\\Work\\2021\\20210201_ホメ春香\\女踊りジグザグ右足始まり0000-1440f.vmd"
        )
        motion = reader.read_data()
        ...
        for bone_name, bf_list in motion.bones.items():
            for fno, bf in bf_list.items():
                x_qq, y_qq, z_qq, _ = MServiceUtils.separate_local_qq(
                    fno, bone_name, bf.rotation, model.get_local_x_axis(bone_name)
                )
        ...
        data_set = MOptionsDataSet(
            ...
            model,
            model,
            "D:\\MMD\\MikuMikuDance_v926x64\\Work\\2021\\20210201_ホメ春香\\女踊りジグザグ右足始まり0000-1440f_ホメ春香.vmd",
        )
        writer = VmdWriter(data_set)
        writer.write()
...
class MorphDataTest(unittest.TestCase):
    def test_bone_morph_check(self):
        ...
        for pmx_path in glob.glob(
            "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Model\\刀剣乱舞\\**\\*.pmx",
            recursive=True,
        ):
            reader = PmxReader(pmx_path, is_check=False)
            model = reader.read_data()
...
class NormalTest(unittest.TestCase):
    def test_normal_bone(self):
        ...
        for bone_name in ["下半身", "上半身", "上半身2", "首", "頭"]:
            ...
            local_x_axis = MVector3D.crossProduct(
                MVector3D(0, 0, 1), local_z_axis
            ).normalized()
            ...
            x_bone.local_z_vector = MVector3D.crossProduct(
                MVector3D(0, 0, 1), local_x_axis
            ).normalized()
            ...
            y_bone.local_z_vector = MVector3D.crossProduct(
                MVector3D(0, 0, 1), local_y_axis
            ).normalized()
...
class SdefDataTest(unittest.TestCase):
    def test_sdef_check(self):
        ...
        for pmx_path in glob.glob(
            "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Model\\VOCALOID\\**\\*.pmx",
            recursive=True,
        ):
            reader = PmxReader(pmx_path, is_check=False)
            model = reader.read_data()
            ...
            for bone_idx, vertices in model.vertices.items():
                for vertex in vertices:
                    if type(vertex.deform) is Sdef:
                        logger.warning(
                            "SDEFありモデル: %s, idx: %s, bone: %s",
                            pmx_path,
                            vertex.index,
                            model.bone_indexes[bone_idx],
                        )
                        is_sdef = True
                        break
...
class PmxDataTest(unittest.TestCase):
    def test_get_wrist_vertex_01(self):
        model = PmxReader(
            "D:/MMD/MikuMikuDance_v926x64/UserFile/Model/_VMDサイジング/8頭身審神者 猫のしもべ/8頭身審神者3_軸制限無し.pmx"
        ).read_data()
    ...
    def test_get_wrist_vertex_02(self):
        model = PmxReader(
            "D:/MMD/MikuMikuDance_v926x64/UserFile/Model/刀剣乱舞/047_五虎退/五虎退 りっつ式 v1.42/五虎退.pmx"
        ).read_data()
    ...
    def test_create_link_2_top_one_01(self):
        pmx_data = PmxModel()
        pmx_data.bones["SIZING_ROOT_BONE"] = Bone(
...
class VmdDataTest(unittest.TestCase):
    ...
    def test_calc_bf_02(self):
        motion = VmdReader("test/data/補間曲線テスト01.vmd").read_data()
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 60.00003815, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.000829206, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.000238923, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.000410508, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.007236294, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.002085242, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.003582553, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.026710032, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.007699313, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.013225263, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.069654942, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.020092424, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.034498487, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.151426569, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.043738037, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.07503701, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.293533772, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.084980235, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.145587772, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.529829144, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.153979659, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.263184816, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 0.91197902, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.266692847, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.454135865, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 1.525828362, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.450684816, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 0.762894511, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 2.53502202, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 0.761205316, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 1.276175618, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 4.252967358, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 1.313556314, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 2.167253256, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 7.438279152, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 2.422097683, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 3.884207487, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 14.49954891, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 5.332914352, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 8.068556786, delta=0.1
        )
        ...
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().x(), 33.70291519, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().y(), 18.53442383, delta=0.1
        )
        self.assertAlmostEqual(
            bf.rotation.toEulerAngles4MMD().z(), 24.47537041, delta=0.1
        )
    ...
    def test_split_bf_by_fno(self):
        ...
        for fno in motion.get_bone_fnos(target_bone_name):
            ...
            print(
                "int move x: %s, %s, %s, %s"
                % (
                    bf.interpolation[MBezierUtils.MX_x1_idxs[3]],
                    bf.interpolation[MBezierUtils.MX_y1_idxs[3]],
                    bf.interpolation[MBezierUtils.MX_x2_idxs[3]],
                    bf.interpolation[MBezierUtils.MX_y2_idxs[3]],
                )
            )
            print(
                "int move y: %s, %s, %s, %s"
                % (
                    bf.interpolation[MBezierUtils.MY_x1_idxs[3]],
                    bf.interpolation[MBezierUtils.MY_y1_idxs[3]],
                    bf.interpolation[MBezierUtils.MY_x2_idxs[3]],
                    bf.interpolation[MBezierUtils.MY_y2_idxs[3]],
                )
            )
            print(
                "int move z: %s, %s, %s, %s"
                % (
                    bf.interpolation[MBezierUtils.MZ_x1_idxs[3]],
                    bf.interpolation[MBezierUtils.MZ_y1_idxs[3]],
                    bf.interpolation[MBezierUtils.MZ_x2_idxs[3]],
                    bf.interpolation[MBezierUtils.MZ_y2_idxs[3]],
                )
            )
            print(
                "int rot: %s, %s, %s, %s"
                % (
                    bf.interpolation[MBezierUtils.R_x1_idxs[3]],
                    bf.interpolation[MBezierUtils.R_y1_idxs[3]],
                    bf.interpolation[MBezierUtils.R_x2_idxs[3]],
                    bf.interpolation[MBezierUtils.R_y2_idxs[3]],
                )
            )
    ...
    def test_split_bf_by_fno01(self):
        original_motion = VmdReader("test/data/補間曲線テスト01.vmd").read_data()
        model = PmxReader(
            "D:/MMD/MikuMikuDance_v926x64/UserFile/Model/ダミーボーン頂点追加2.pmx"
        ).read_data()
        ...
        for pidx in range(10):
            try:
                ...
                for fill_fno in range(
                    ...
                    motion.reset_interpolation_parts(
                        target_bone_name,
                        next_bf,
                        [None, MVector2D(20, 20), MVector2D(107, 107), None],
                        MBezierUtils.R_x1_idxs,
                        MBezierUtils.R_y1_idxs,
                        MBezierUtils.R_x2_idxs,
                        MBezierUtils.R_y2_idxs,
                    )
                    motion.reset_interpolation_parts(
                        target_bone_name,
                        next_bf,
                        [
                            None,
                            MVector2D(params[0][0], params[0][1]),
                            MVector2D(params[0][2], params[0][3]),
                            None,
                        ],
                        MBezierUtils.MX_x1_idxs,
                        MBezierUtils.MX_y1_idxs,
                        MBezierUtils.MX_x2_idxs,
                        MBezierUtils.MX_y2_idxs,
                    )
                    motion.reset_interpolation_parts(
                        target_bone_name,
                        next_bf,
                        [None, MVector2D(20, 20), MVector2D(107, 107), None],
                        MBezierUtils.MY_x1_idxs,
                        MBezierUtils.MY_y1_idxs,
                        MBezierUtils.MY_x2_idxs,
                        MBezierUtils.MY_y2_idxs,
                    )
                    motion.reset_interpolation_parts(
                        target_bone_name,
                        next_bf,
                        [None, MVector2D(20, 20), MVector2D(107, 107), None],
                        MBezierUtils.MZ_x1_idxs,
                        MBezierUtils.MZ_y1_idxs,
                        MBezierUtils.MZ_x2_idxs,
                        MBezierUtils.MZ_y2_idxs,
                    )
                    ...
                    result = motion.split_bf_by_fno(
                        target_bone_name, prev_bf, next_bf, fill_fno
                    )
                    ...
                    for now_fno in motion.get_bone_fnos(target_bone_name):
                        ...
                        print(
                            "int move x: %s, %s, %s, %s"
                            % (
                                now_bf.interpolation[MBezierUtils.MX_x1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MX_y1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MX_x2_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MX_y2_idxs[3]],
                            )
                        )
                        print(
                            "int move y: %s, %s, %s, %s"
                            % (
                                now_bf.interpolation[MBezierUtils.MY_x1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MY_y1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MY_x2_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MY_y2_idxs[3]],
                            )
                        )
                        print(
                            "int move z: %s, %s, %s, %s"
                            % (
                                now_bf.interpolation[MBezierUtils.MZ_x1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MZ_y1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MZ_x2_idxs[3]],
                                now_bf.interpolation[MBezierUtils.MZ_y2_idxs[3]],
                            )
                        )
                        print(
                            "int rot: %s, %s, %s, %s"
                            % (
                                now_bf.interpolation[MBezierUtils.R_x1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.R_y1_idxs[3]],
                                now_bf.interpolation[MBezierUtils.R_x2_idxs[3]],
                                now_bf.interpolation[MBezierUtils.R_y2_idxs[3]],
                            )
                        )
                    ...
                    data_set = MOptionsDataSet(
                        ...
                        model,
                        model,
                        "E:/WebDownload/test_split_bf_by_fno01_{0:%Y%m%d_%H%M%S%f}.vmd".format(
                            now
                        ),
                        False,
                        False,
                    ...
                    data_set = MOptionsDataSet(
                        ...
                        model,
                        model,
                        "E:/WebDownload/test_split_bf_by_fno01_{0:%Y%m%d_%H%M%S%f}_orignal.vmd".format(
                            now
                        ),
                        False,
                        False,
    ...
    def a_test_split_bf_by_fno02(self):
        ...
        for xparams, yparams, zparams, rparams in zip(
            np.array(
                np.meshgrid(base_params, base_params, base_params, base_params)
            ).T.reshape(-1, 4),
            np.array(
                np.meshgrid(base_params, base_params, base_params, base_params)
            ).T.reshape(-1, 4),
            ...
            try:
                for fill_fno in range(
                    ...
                    motion.reset_interpolation_parts(
                        ...
                        [
                            None,
                            MVector2D(xparams[0], xparams[1]),
                            MVector2D(xparams[2], xparams[3]),
                            None,
                        ],
                        MBezierUtils.MX_x1_idxs,
                        MBezierUtils.MX_y1_idxs,
                    ...
                    motion.reset_interpolation_parts(
                        ...
                        [
                            None,
                            MVector2D(yparams[0], yparams[1]),
                            MVector2D(yparams[2], yparams[3]),
                            None,
                        ],
                        MBezierUtils.MY_x1_idxs,
                        MBezierUtils.MY_y1_idxs,
                    ...
                    motion.reset_interpolation_parts(
                        ...
                        [
                            None,
                            MVector2D(zparams[0], zparams[1]),
                            MVector2D(zparams[2], zparams[3]),
                            None,
                        ],
                        MBezierUtils.MZ_x1_idxs,
                        MBezierUtils.MZ_y1_idxs,
                    ...
                    motion.reset_interpolation_parts(
                        ...
                        [
                            None,
                            MVector2D(rparams[0], rparams[1]),
                            MVector2D(rparams[2], rparams[3]),
                            None,
                        ],
                        MBezierUtils.R_x1_idxs,
                        MBezierUtils.R_y1_idxs,
