        ...
        try:
            ...
            for org_morph_name, rep_morph_name, morph_ratio in data_set.morph_list:
                if org_morph_name in data_set.motion.morphs:
                    # 置換元モーフがある場合、保持
                    original_morphs[org_morph_name] = copy.deepcopy(
                        data_set.motion.morphs[org_morph_name]
                    )

                if (
                    rep_morph_name in data_set.motion.morphs
                    and org_morph_name != rep_morph_name
                ):
                    # 置換先モーフがある場合、保持（元と先が同じ場合、スルー）
                    replaced_morphs[rep_morph_name] = copy.deepcopy(
                        data_set.motion.morphs[rep_morph_name]
                    )
            ...
            for org_morph_name, rep_morph_name, morph_ratio in data_set.morph_list:
                if (
                    org_morph_name in original_morphs
                    and rep_morph_name not in replaced_morphs
                ):
                    ...
                    org_morph_name in original_morphs
                    and rep_morph_name in replaced_morphs
                ):
                    # 元にあって、先にもある場合
                    for fno, org_morph_data in original_morphs[org_morph_name].items():
                        morph_data = VmdMorphFrame(fno)
                        ...
                        if fno in replaced_morphs[rep_morph_name]:
                            ...
                            morph_data.ratio += replaced_morphs[rep_morph_name][
                                fno
                            ].ratio
