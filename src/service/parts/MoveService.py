...
class MoveService:
    ...
    def execute(self):
        ...
        with ThreadPoolExecutor(
            thread_name_prefix="move", max_workers=min(5, self.options.max_workers)
        ) as executor:
            for data_set_idx, data_set in enumerate(self.options.data_set_list):
                ...
                for bone_name in [
                    ...
                    if (
                        bone_name in data_set.motion.bones
                        and bone_name in data_set.rep_model.bones
                        and len(data_set.motion.bones[bone_name].keys()) > 0
                    ):
                        futures.append(
                            executor.submit(self.adjust_move, data_set_idx, bone_name)
                        )
                ...
                if self.options.now_process_ctrl:
                    ...
                    proccess_key = "【No.{0}】{1}({2})".format(
                        data_set_idx + 1,
                        os.path.basename(data_set.motion.path),
                        data_set.rep_model.name,
                    )
    ...
    def adjust_move(self, data_set_idx: int, bone_name: str):
        try:
            ...
            for fno in fnos:
                ...
                _, rep_global_mats = MServiceUtils.calc_global_pos(
                    data_set.rep_model,
                    bone_link,
                    data_set.motion,
                    fno,
    ...
    def set_leg_ik_offset(self, data_set_idx: int, data_set: MOptionsDataSet):
        ...
        if set(target_bones).issubset(data_set.org_model.bones) and set(
            target_bones
        ).issubset(data_set.rep_model.bones):
            ...
            for direction in ["左", "右"]:
                org_leg_ik_pos = data_set.org_model.bones[
                    "{0}足ＩＫ".format(direction)
                ].position
                rep_leg_ik_global_pos = data_set.rep_model.bones[
                    "{0}足ＩＫ".format(direction)
                ].position
                org_leg_pos = data_set.org_model.bones[
                    "{0}足".format(direction)
                ].position
                rep_leg_pos = data_set.rep_model.bones[
                    "{0}足".format(direction)
                ].position
                # IKオフセット
                leg_ik_offset[direction] = (
                    (org_leg_ik_pos - org_leg_pos) * leg_ratio
                ) - (rep_leg_ik_global_pos - rep_leg_pos)
                leg_ik_offset[direction].effective()
                leg_ik_offset[direction].setY(0)
                leg_ik_offset[direction].setZ(0)
                logger.debug(
                    "leg_ik_offset(%s): %s", direction, leg_ik_offset[direction]
                )

                if abs(leg_ik_offset[direction].x() * data_set.original_xz_ratio) > abs(
                    rep_leg_ik_global_pos.x()
                ):
                    # IKオフセットが、元々の足の位置の一定以上に広がっている場合、縮める
                    re_x = (
                        rep_leg_ik_global_pos.x()
                        - (leg_ik_offset[direction].x() * data_set.original_xz_ratio)
                    ) * data_set.original_xz_ratio
                    # オフセットの広がり具合が、元々と同じ場合は正、反対の場合、負
                    leg_ik_offset[direction].setX(
                        re_x
                        * (
                            1
                            if np.sign(leg_ik_offset[direction].x())
                            == np.sign(rep_leg_ik_global_pos.x())
                            else -1
                        )
                    )
                ...
                if data_set_idx in self.options.leg_options.leg_offsets:
                    specified_leg_offset = self.options.leg_options.leg_offsets[
                        data_set_idx
                    ] * np.sign(rep_leg_ik_global_pos.x())

                leg_ik_offset[direction].setX(
                    leg_ik_offset[direction].x() + specified_leg_offset
                )
                logger.debug(
                    "specified_leg_offset(%s): %s -> %s",
                    direction,
                    specified_leg_offset,
                    leg_ik_offset[direction].x(),
                )

            logger.info(
                "【No.%s】IKオフセット(%s): x: %s, z: %s",
                (data_set_idx + 1),
                "左足",
                leg_ik_offset["左"].x(),
                leg_ik_offset["左"].z(),
            ...
            if (
                "左足IK親" in data_set.rep_model.bones
                and "左足IK親" in data_set.motion.bones
            ):
            ...
            if (
                "右足IK親" in data_set.rep_model.bones
                and "右足IK親" in data_set.motion.bones
            ):
                # IK親があって使われている場合、IK親にオフセット設定
    ...
    def set_center_y_offset(self, data_set_idx: int, data_set: MOptionsDataSet):
        ...
        if set(target_bones).issubset(data_set.org_model.bones) and set(
            ...
            rep_ankle_pos = data_set.rep_model.bones["左足首"].position - MVector3D(
                specified_leg_offset, 0, 0
            )
            # 元モデルの足の長さ比
            org_leg_upper_length = data_set.org_model.bones[
                "左ひざ"
            ].position.distanceToPoint(data_set.org_model.bones["左足"].position)
            org_leg_lower_length = data_set.org_model.bones[
                "左ひざ"
            ].position.distanceToPoint(rep_ankle_pos)
            org_leg_ik_length = (
                data_set.org_model.bones["左足"].position - rep_ankle_pos
            ).y()
            ...
            # 足ボーンの長さとIKの長さ比
            org_leg_ratio = org_leg_ik_length / (
                org_leg_upper_length + org_leg_lower_length
            )
            logger.test("org_leg_ratio: %s", org_leg_ratio)

            # 先モデルの足の長さ比
            rep_leg_upper_length = data_set.rep_model.bones[
                "左ひざ"
            ].position.distanceToPoint(data_set.rep_model.bones["左足"].position)
            rep_leg_lower_length = data_set.rep_model.bones[
                "左ひざ"
            ].position.distanceToPoint(data_set.rep_model.bones["左足首"].position)
            rep_leg_ik_length = (
                data_set.rep_model.bones["左足"].position
                - data_set.rep_model.bones["左足首"].position
            ).y()
            ...
            rep_recalc_ik_length = org_leg_ratio * (
                rep_leg_upper_length + rep_leg_lower_length
            )
            ...
            if rep_recalc_ik_length < rep_leg_ik_length:
                ...
                logger.test(
                    "local_offset %s", data_set.rep_model.bones["センター"].local_offset
                )

                logger.info(
                    "【No.%s】センターYオフセット: %s", (data_set_idx + 1), offset_y
                )
    ...
    def set_center_z_offset(self, data_set_idx: int, data_set: MOptionsDataSet):
        ...
        if set(target_bones).issubset(data_set.org_model.bones) and set(
            ...
            logger.test(
                "org_center_gravity %s, org_leg_zlength: %s",
                org_center_gravity,
                org_leg_zlength,
            )
            ...
            logger.test(
                "rep_center_gravity %s, rep_leg_zlength: %s",
                rep_center_gravity,
                rep_leg_zlength,
            )

            local_offset_z = (rep_center_gravity - org_center_gravity) * (
                rep_leg_zlength / org_leg_zlength
            )
            ...
            logger.info(
                "【No.%s】センターZオフセット: %s", (data_set_idx + 1), local_offset_z
            )
