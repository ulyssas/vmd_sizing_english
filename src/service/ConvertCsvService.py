...
class ConvertCsvService:
    ...
    def execute(self):
        ...
        try:
            ...
            service_data_txt = "{service_data_txt}　　VMD: {vmd}\n".format(
                ...
                vmd=os.path.basename(self.options.motion.path),
            )  # noqa
            ...
            logger.error(
                "CSV変換処理が処理できないデータで終了しました。\n\n%s",
                se.message,
                decoration=MLogger.DECORATION_BOX,
            )
        except Exception:
            logger.critical(
                "CSV変換処理が意図せぬエラーで終了しました。\n\n%s",
    ...
    def convert_csv(self):
        ...
        if (
            self.options.motion.motion_cnt
            == self.options.motion.morph_cnt
            == self.options.motion.camera_cnt
            == 0
        ):
            logger.warning(
                "出力可能なモーションデータ（ボーン・モーフ・カメラ）がありません",
                decoration=MLogger.DECORATION_BOX,
            )
        ...
        if self.options.motion.motion_cnt > 0:
            ...
            logger.info(
                "ボーンモーションCSV: %s", bone_fpath, decoration=MLogger.DECORATION_BOX
            )
