    ...
    def execute(self):
        ...
        try:
            ...
            service_data_txt = "{service_data_txt}　　ボーンCSV: {bone_csv}\n".format(
                ...
                bone_csv=os.path.basename(self.options.bone_csv_path),
            )  # noqa
            service_data_txt = "{service_data_txt}　　モーフCSV: {morph_csv}\n".format(
                service_data_txt=service_data_txt,
                morph_csv=os.path.basename(self.options.morph_csv_path),
            )  # noqa
            service_data_txt = "{service_data_txt}　　カメラCSV: {camera_csv}\n".format(
                service_data_txt=service_data_txt,
                camera_csv=os.path.basename(self.options.camera_csv_path),
            )  # noqa
            ...
            logger.error(
                "VMD変換処理が処理できないデータで終了しました。\n\n%s",
                se.message,
                decoration=MLogger.DECORATION_BOX,
            )
        except Exception:
            logger.critical(
                "VMD変換処理が意図せぬエラーで終了しました。\n\n%s",
    ...
    def convert_vmd(self):
            ...
            with open(self.options.bone_csv_path, encoding="cp932", mode="r") as f:
                ...
                for ridx, row in enumerate(reader):
                    ...
                    try:
                        ...
                        if bf.fno < 0:
                            logger.error(
                                "[ボーン] %s行目のフレーム番号（2列目）に負数が設定されています",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
                    ...
                    try:
                        if len(row) < 4 or not row[2] or not row[3] or not row[4]:
                            logger.error(
                                "[ボーン] %s行目の位置（3-5列目）のいずれかが設定されていません",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
                    ...
                    try:
                        if len(row) < 7 or not row[5] or not row[6] or not row[7]:
                            logger.error(
                                "[ボーン] %s行目の回転（6-8列目）のいずれかが設定されていません",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
                        ...
                        bf.rotation = MQuaternion.fromEulerAngles(
                            float(row[5]), float(row[6]) * -1, float(row[7]) * -1
                        )
                    except Exception as e:
                        logger.error(
            ...
            with open(self.options.morph_csv_path, encoding="cp932", mode="r") as f:
                ...
                for ridx, row in enumerate(reader):
                    ...
                    try:
                        ...
                        if mf.fno < 0:
                            logger.error(
                                "[モーフ] %s行目のフレーム番号（2列目）に負数が設定されています",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
        ...
        if len(bone_motion.bones.keys()) > 0 or len(bone_motion.morphs.keys()) > 0:
            ...
            data_set = MOptionsDataSet(
                bone_motion, model, model, bone_fpath, False, False, [], None, 0, []
            )
            ...
            with open(self.options.camera_csv_path, encoding="cp932", mode="r") as f:
                ...
                for ridx, row in enumerate(reader):
                    ...
                    try:
                        ...
                        if cf.fno < 0:
                            logger.error(
                                "[カメラ] %s行目のフレーム番号（1列目）に負数が設定されています",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
                    ...
                    try:
                        if len(row) < 3 or not row[1] or not row[2] or not row[3]:
                            logger.error(
                                "[カメラ] %s行目の位置（2-4列目）のいずれかが設定されていません",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
                    ...
                    try:
                        if len(row) < 6 or not row[4] or not row[5] or not row[6]:
                            logger.error(
                                "[カメラ] %s行目の回転（5-7列目）のいずれかが設定されていません",
                                rno,
                                decoration=MLogger.DECORATION_BOX,
                            )
                            return False
