    ...
    def execute(self):
        ...
        try:
            ...
            for data_set_idx, data_set in enumerate(self.options.data_set_list):
                service_data_txt = (
                    "{service_data_txt}\n【No.{no}】 --------- \n".format(
                        service_data_txt=service_data_txt, no=(data_set_idx + 1)
                    )
                )  # noqa
                service_data_txt = (
                    "{service_data_txt}　　モーション: {motion}\n".format(
                        service_data_txt=service_data_txt,
                        motion=os.path.basename(data_set.motion.path),
                    )
                )  # noqa
                service_data_txt = "{service_data_txt}　　作成元モデル: {trace_model} ({model_name})\n".format(
                    service_data_txt=service_data_txt,
                    trace_model=os.path.basename(data_set.org_model.path),
                    model_name=data_set.org_model.name,
                )  # noqa
                service_data_txt = "{service_data_txt}　　変換先モデル: {replace_model} ({model_name})\n".format(
                    service_data_txt=service_data_txt,
                    replace_model=os.path.basename(data_set.rep_model.path),
                    model_name=data_set.rep_model.name,
                )  # noqa
                if data_set.camera_org_model:
                    service_data_txt = "{service_data_txt}　　カメラ作成元モデル: {trace_model} ({model_name})\n".format(
                        service_data_txt=service_data_txt,
                        trace_model=os.path.basename(data_set.camera_org_model.path),
                        model_name=data_set.camera_org_model.name,
                    )  # noqa
                    service_data_txt = (
                        "{service_data_txt}　　Yオフセット: {camera_offset_y}\n".format(
                            service_data_txt=service_data_txt,
                            camera_offset_y=data_set.camera_offset_y,
                        )
                    )  # noqa
                service_data_txt = "{service_data_txt}　　スタンス追加補正有無: {detail_stance_flg}\n".format(
                    service_data_txt=service_data_txt,
                    detail_stance_flg=data_set.detail_stance_flg,
                )  # noqa
                if data_set.detail_stance_flg:
                    # スタンス追加補正がある場合、そのリストを表示
                    service_data_txt = (
                        "{service_data_txt}　　　　{detail_stance_flg}\n".format(
                            service_data_txt=service_data_txt,
                            detail_stance_flg=", ".join(
                                data_set.selected_stance_details
                            ),
                        )
                    )  # noqa
                ...
                service_data_txt = (
                    "{service_data_txt}　　捩り分散有無: {twist_flg}\n".format(
                        service_data_txt=service_data_txt, twist_flg=data_set.twist_flg
                    )
                )  # noqa
                ...
                for org_morph_name, rep_morph_name, morph_ratio in data_set.morph_list:
                    morph_list.append(
                        f"{org_morph_name} → {rep_morph_name} ({morph_ratio})"
                    )
                morph_txt = ", ".join(morph_list)
                service_data_txt = (
                    "{service_data_txt}　　モーフ置換: {morph_txt}\n".format(
                        service_data_txt=service_data_txt, morph_txt=morph_txt
                    )
                )  # noqa

                if data_set_idx in self.options.arm_options.avoidance_target_list:
                    service_data_txt = (
                        "{service_data_txt}　　対象剛体名: {avoidance_target}\n".format(
                            service_data_txt=service_data_txt,
                            avoidance_target=", ".join(
                                self.options.arm_options.avoidance_target_list[
                                    data_set_idx
                                ]
                            ),
                        )
            ...
            if self.options.arm_options.avoidance:
                service_data_txt = (
                    "{service_data_txt}剛体接触回避: {avoidance}\n".format(
                        service_data_txt=service_data_txt,
                        avoidance=self.options.arm_options.avoidance,
                    )
                )  # noqa
            ...
            if self.options.arm_options.alignment:
                service_data_txt = "{service_data_txt}手首位置合わせ: {alignment} ({distance})\n".format(
                    service_data_txt=service_data_txt,
                    alignment=self.options.arm_options.alignment,
                    distance=self.options.arm_options.alignment_distance_wrist,
                )  # noqa
                service_data_txt = (
                    "{service_data_txt}指位置合わせ: {alignment} ({distance})\n".format(
                        service_data_txt=service_data_txt,
                        alignment=self.options.arm_options.alignment_finger_flg,
                        distance=self.options.arm_options.alignment_distance_finger,
                    )
                )  # noqa
                service_data_txt = (
                    "{service_data_txt}床位置合わせ: {alignment} ({distance})\n".format(
                        service_data_txt=service_data_txt,
                        alignment=self.options.arm_options.alignment_floor_flg,
                        distance=self.options.arm_options.alignment_distance_floor,
                    )
                )  # noqa

            if self.options.arm_options.arm_check_skip_flg:
                service_data_txt = (
                    "{service_data_txt}腕チェックスキップ: {arm_check_skip}\n".format(
                        service_data_txt=service_data_txt,
                        arm_check_skip=self.options.arm_options.arm_check_skip_flg,
                    )
                )  # noqa

            if self.options.camera_motion:
                service_data_txt = (
                    "{service_data_txt}カメラ: {camera}({camera_length})\n".format(
                        service_data_txt=service_data_txt,
                        camera=os.path.basename(self.options.camera_motion.path),
                        camera_length=self.options.camera_length,
                    )
                )  # noqa
                service_data_txt = "{service_data_txt}　　距離制限: {camera_length}{camera_length_umlimit}\n".format(
                    service_data_txt=service_data_txt,
                    camera_length=self.options.camera_length,
                    camera_length_umlimit=(
                        "" if self.options.camera_length < 5 else "(無制限)"
                    ),
                )  # noqa

            service_data_txt = "{service_data_txt}------------------------".format(
                service_data_txt=service_data_txt
            )  # noqa
            ...
            if self.options.is_sizing_camera_only is True:
                ...
                for data_set_idx, data_set in enumerate(self.options.data_set_list):
                    ...
                    (
                        ...
                        data_set.original_y_ratio,
                        data_set.original_heads_tall_ratio,
                    ) = MServiceUtils.calc_leg_ik_ratio(data_set)
                ...
                for data_set_idx, data_set in enumerate(self.options.data_set_list):
                    ...
                    try:
                        ...
                        logger.info(
                            "【No.%s】 出力終了: %s",
                            (data_set_idx + 1),
                            os.path.basename(data_set.output_vmd_path),
                            decoration=MLogger.DECORATION_BOX,
                            title="サイジング成功",
                        )
                    except FileNotFoundError as fe:
                        logger.error(
                            "【No.%s】出力VMDファイルが正常に作成されなかったようです。\nパスを確認してください。%s\n\n%s",
                            (data_set_idx + 1),
                            data_set.output_vmd_path,
                            fe,
                            decoration=MLogger.DECORATION_BOX,
                        )
            ...
            if self.options.camera_motion:
                try:
                    ...
                    data_set = MOptionsDataSet(
                        ...
                        None,
                        camera_model,
                        self.options.camera_output_vmd_path,
                        0,
                        0,
                        [],
                        None,
