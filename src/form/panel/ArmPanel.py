...
class ArmPanel(BasePanel):
    def __init__(self, frame: wx.Frame, parent: wx.Notebook, tab_idx: int):
        ...
        avoidance_tooltip = (
            "Avoid contact between bone-tracking rigid bodies with the specified name and the wrist/fingertips.\n"
        ...
        self.description_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            'You can adjust the arms to match the target model.\nYou can execute both "Contact Avoidance" and "Alignment" together. (Contact Avoidance → Alignment)'
            + "\nArm movement may change from the original motion. Both processes take some time.",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        ...
        # 剛体接触回避タイトル
        self.avoidance_title_txt = wx.StaticText(
            self, wx.ID_ANY, "Contact Avoidance", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.avoidance_title_txt.SetToolTip(avoidance_tooltip)
        self.avoidance_title_txt.Wrap(-1)
        self.avoidance_title_txt.SetFont(
            wx.Font(
        ...
        self.avoidance_title_txt.Bind(
            wx.EVT_LEFT_DOWN, self.on_check_arm_process_avoidance
        )

        self.arm_process_flg_avoidance = wx.CheckBox(
        ...
        # 剛体接触回避説明文
        self.avoidance_description_txt = wx.StaticText(
            self, wx.ID_ANY, avoidance_tooltip, wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        # 剛体名指定
        self.avoidance_target_txt_ctrl = wx.TextCtrl(
            self,
            wx.ID_ANY,
            "",
            wx.DefaultPosition,
        ...
        self.avoidance_target_txt_ctrl.SetBackgroundColour(
            wx.SystemSettings.GetColour(wx.SYS_COLOUR_3DLIGHT)
        )
        self.avoidance_target_txt_ctrl.Bind(
            wx.EVT_TEXT, self.on_check_arm_process_avoidance
        )
        self.avoidance_target_sizer.Add(
            self.avoidance_target_txt_ctrl, 1, wx.EXPAND | wx.ALL, 5
        )

        self.avoidance_target_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "Select Rigid Body", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.avoidance_target_btn_ctrl.SetToolTip(
            "You can select bone-tracking rigid bodies in the target model"
        )
        self.avoidance_target_btn_ctrl.Bind(
            wx.EVT_BUTTON, self.on_click_avoidance_target
        )
        self.avoidance_target_sizer.Add(
            self.avoidance_target_btn_ctrl, 0, wx.ALIGN_BOTTOM | wx.ALL, 5
        )
        ...
        # 手首位置合わせタイトル
        self.alignment_title_txt = wx.StaticText(
            self, wx.ID_ANY, "Alignment", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.alignment_title_txt.SetToolTip(
            "Adjust motions such as joining hands or touching the floor, to match the wrist position of the target model.\n"
            + "You can adjust the effective range of alignment by adjusting each distance."
        )
        self.alignment_title_txt.Wrap(-1)
        self.alignment_title_txt.SetFont(
            wx.Font(
                wx.NORMAL_FONT.GetPointSize(),
                wx.FONTFAMILY_DEFAULT,
                wx.FONTSTYLE_NORMAL,
        ...
        self.alignment_title_txt.Bind(
            wx.EVT_LEFT_DOWN, self.on_check_arm_process_alignment
        )

        self.arm_process_flg_alignment = wx.CheckBox(
            self, wx.ID_ANY, "", wx.DefaultPosition, wx.DefaultSize
        )
        ...
        # 手首位置合わせ説明文
        self.alignment_description_txt = wx.StaticText(
            self, wx.ID_ANY, alignment_tooltip, wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        # 指位置合わせ
        self.arm_alignment_finger_flg_ctrl = wx.CheckBox(
            self,
            wx.ID_ANY,
            "Align by finger position",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        self.arm_alignment_finger_flg_ctrl.SetToolTip(
            "If checked, you can adjust the wrist position based on the distance between fingers, useful for finger-tutting motions.\nFor multi-person motions, it's better to leave it OFF for cleaner results."
        )
        self.arm_alignment_finger_flg_ctrl.Bind(
            wx.EVT_CHECKBOX, self.on_check_arm_process_alignment
        )
        self.alignment_option_sizer.Add(
            self.arm_alignment_finger_flg_ctrl, 0, wx.ALL, 5
        )

        # 床位置合わせ
        self.arm_alignment_floor_flg_ctrl = wx.CheckBox(
            self,
            wx.ID_ANY,
            "Align with the floor as well",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        self.arm_alignment_floor_flg_ctrl.SetToolTip(
            "If checked, you can adjust the wrist position to match the source model when the wrist sinks into or floats above the floor.\nThe center position will also be adjusted together."
        )
        self.arm_alignment_floor_flg_ctrl.Bind(
            wx.EVT_CHECKBOX, self.on_check_arm_process_alignment
        )
        ...
        self.alignment_distance_wrist_txt = wx.StaticText(
            self, wx.ID_ANY, "Wrist Distance    ", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.alignment_distance_wrist_txt.SetToolTip(
            "Specify how close the wrists should be to execute wrist alignment.\nSmaller values mean alignment happens only when wrists are close.\nThe unit of distance is the size of the palm of the source model."
        ...
        self.alignment_distance_wrist_sizer.Add(
            self.alignment_distance_wrist_txt, 0, wx.ALL, 5
        )

        self.alignment_distance_wrist_label = wx.StaticText(
            self, wx.ID_ANY, "(1.7)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.alignment_distance_wrist_label.SetToolTip(
            "The currently specified wrist distance. If the wrist positions of the source model are within this range, wrist alignment will be performed."
        )
        self.alignment_distance_wrist_label.Wrap(-1)
        self.alignment_distance_wrist_sizer.Add(
            self.alignment_distance_wrist_label, 0, wx.ALL, 5
        )

        self.alignment_distance_wrist_slider = FloatSliderCtrl(
            self,
            wx.ID_ANY,
            1.7,
            0,
            10,
            0.1,
            self.alignment_distance_wrist_label,
            wx.DefaultPosition,
            wx.DefaultSize,
            wx.SL_HORIZONTAL,
        )
        self.alignment_distance_wrist_slider.Bind(
            wx.EVT_SCROLL_CHANGED, self.on_check_arm_process_alignment
        )
        self.alignment_distance_wrist_sizer.Add(
            self.alignment_distance_wrist_slider, 1, wx.ALL | wx.EXPAND, 5
        )
        ...
        self.alignment_distance_finger_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            "Finger Distance      ",
            wx.DefaultPosition,
        ...
        self.alignment_distance_finger_txt.SetToolTip(
            "Specify how close the fingers should be to execute finger alignment.\nSmaller values mean alignment happens only when fingers are close.\nThe unit of distance is the size of the palm of the source model.\n"
        ...
        self.alignment_distance_finger_sizer.Add(
            self.alignment_distance_finger_txt, 0, wx.ALL, 5
        )

        self.alignment_distance_finger_label = wx.StaticText(
            self, wx.ID_ANY, "(1.4)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.alignment_distance_finger_label.SetToolTip(
            "The currently specified finger distance. If the finger positions of the source model are within this range, finger alignment will be performed."
        )
        self.alignment_distance_finger_label.Wrap(-1)
        self.alignment_distance_finger_sizer.Add(
            self.alignment_distance_finger_label, 0, wx.ALL, 5
        )

        self.alignment_distance_finger_slider = FloatSliderCtrl(
            self,
            wx.ID_ANY,
            1.4,
            0,
            10,
            0.1,
            self.alignment_distance_finger_label,
            wx.DefaultPosition,
            wx.DefaultSize,
            wx.SL_HORIZONTAL,
        )
        self.alignment_distance_finger_slider.Bind(
            wx.EVT_SCROLL_CHANGED, self.on_check_arm_process_alignment
        )
        self.alignment_distance_finger_sizer.Add(
            self.alignment_distance_finger_slider, 1, wx.ALL | wx.EXPAND, 5
        )
        ...
        self.alignment_distance_floor_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            "Wrist-Floor Distance",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        self.alignment_distance_floor_txt.SetToolTip(
            "Specify how close the wrist and floor should be to execute wrist-floor alignment.\nSmaller values mean alignment is performed only when wrist and floor are close.\nThe unit of distance is the size of the palm of the source model."
        ...
        self.alignment_distance_floor_sizer.Add(
            self.alignment_distance_floor_txt, 0, wx.ALL, 5
        )

        self.alignment_distance_floor_label = wx.StaticText(
            self, wx.ID_ANY, "(1.2)", wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.alignment_distance_floor_label.SetToolTip(
            "The currently specified wrist-floor distance. If the wrist-floor distance of the source model is within this range, wrist-floor alignment will be performed."
        )
        self.alignment_distance_floor_label.Wrap(-1)
        self.alignment_distance_floor_sizer.Add(
            self.alignment_distance_floor_label, 0, wx.ALL, 5
        )

        self.alignment_distance_floor_slider = FloatSliderCtrl(
            self,
            wx.ID_ANY,
            1.2,
            0,
            10,
            0.1,
            self.alignment_distance_floor_label,
            wx.DefaultPosition,
            wx.DefaultSize,
            wx.SL_HORIZONTAL,
        )
        self.alignment_distance_floor_slider.Bind(
            wx.EVT_SCROLL_CHANGED, self.on_check_arm_process_alignment
        )
        self.alignment_distance_floor_sizer.Add(
            self.alignment_distance_floor_slider, 1, wx.ALL | wx.EXPAND, 5
        )

        self.sizer.Add(self.alignment_distance_floor_sizer, 0, wx.ALL | wx.EXPAND, 5)

        self.static_line04 = wx.StaticLine(
            self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize, wx.LI_HORIZONTAL
        )
        ...
        self.arm_check_skip_flg_ctrl = wx.CheckBox(
            self,
            wx.ID_ANY,
            "Skip arm-wrist sizing check",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        self.arm_check_skip_flg_ctrl.SetToolTip(
            "Skip the sizing check (not possible if IKArm/腕IK exists) and always perform processing."
        )
        self.arm_check_skip_sizer.Add(self.arm_check_skip_flg_ctrl, 0, wx.ALL, 5)

        self.arm_check_skip_description = wx.StaticText(
            self,
            wx.ID_ANY,
            "Skip the arm sizing check (not possible if IKArm/腕IK exists) and always perform arm-related processing.\n"
    ...
    def get_avoidance_target(self):
        ...
        if 1 in self.avoidance_set_dict and self.avoidance_set_dict[1].rep_choices:
            if self.avoidance_set_dict[1].equal_hashdigest(
                ...
                target[0] = [
                    self.avoidance_set_dict[1].rep_avoidance_names[n]
                    for n in self.avoidance_set_dict[1].rep_choices.GetSelections()
                ]
            else:
                logger.warning(
                    "No.%s: Contact avoidance settings cleared because the file set was changed after setting contact avoidance.",
                    1,
        ...
        for set_no in list(self.avoidance_set_dict.keys())[1:]:
            if (
                set_no in self.avoidance_set_dict
                and self.avoidance_set_dict[set_no].rep_choices
            ):
                if len(
                    self.frame.multi_panel_ctrl.file_set_list
                ) >= set_no - 1 and self.avoidance_set_dict[set_no].equal_hashdigest(
                    self.frame.multi_panel_ctrl.file_set_list[set_no - 2]
                ):
                    target[set_no - 1] = [
                        self.avoidance_set_dict[set_no].rep_avoidance_names[n]
                        for n in self.avoidance_set_dict[
                            set_no
                        ].rep_choices.GetSelections()
                    ]
                else:
                    logger.warning(
                        "No.%s: Contact avoidance settings cleared because the file set was changed after setting contact avoidance.",
    ...
    def on_click_avoidance_target(self, event: wx.Event):
        ...
        for set_no, set_data in self.avoidance_set_dict.items():
            ...
            if set_data.rep_choices:
                selections = [
                    set_data.rep_choices.GetString(n)
                    for n in set_data.rep_choices.GetSelections()
                ]
                self.avoidance_target_txt_ctrl.WriteText(
        ...
        if self.frame.file_panel_ctrl.file_set.is_loaded():
            if not self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.data.can_arm_sizing:
                ...
                disable_arm_model_names.append(
                    "No.1 Source Model: {0}".format(
                        self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.data.name
                    )
                )
        ...
        for multi_file_set_idx, multi_file_set in enumerate(
            ...
            if multi_file_set.is_loaded():
                if not multi_file_set.org_model_file_ctrl.data.can_arm_sizing:
                    # 腕不可の場合、リスト追加
                    disable_arm_model_names.append(
                        "No.{0} Source Model: {1}".format(
                            set_no, multi_file_set.org_model_file_ctrl.data.name
                        )
                    )
                ...
                if not multi_file_set.rep_model_file_ctrl.data.can_arm_sizing:
                    # 腕不可の場合、リスト追加
                    disable_arm_model_names.append(
                        "No.{0} Target Model: {1}".format(
                            set_no, multi_file_set.rep_model_file_ctrl.data.name
                        )
                    )

        if (
            len(disable_arm_model_names) > 0
            and not self.arm_check_skip_flg_ctrl.GetValue()
        ):
            # 腕不可モデルがいる場合、ダイアログ表示
            with wx.MessageDialog(
                self,
                'The following models contain strings related to "IKArm/腕IK", so arm-related processing for the corresponding file set\n(Arm Stance Correction, Twist Distribution, Contact Avoidance, Alignment) will be skipped as is.\n'
    ...
    def add_set(self, set_idx: int, file_set: SizingFileSet, replace: bool):
        new_avoidance_set = AvoidanceSet(
            self.frame, self, self.avoidance_dialog.scrolled_window, set_idx, file_set
        )
        if replace:
            # 置き換え
            self.avoidance_dialog.set_list_sizer.Hide(
                self.avoidance_set_dict[set_idx].set_sizer, recursive=True
            )
            self.avoidance_dialog.set_list_sizer.Replace(
                self.avoidance_set_dict[set_idx].set_sizer,
                new_avoidance_set.set_sizer,
                recursive=True,
            )
            ...
            # 新規追加
            self.avoidance_dialog.set_list_sizer.Add(
                new_avoidance_set.set_sizer, 0, wx.EXPAND | wx.ALL, 5
            )
        self.avoidance_set_dict[set_idx] = new_avoidance_set

        # スクロールバーの表示のためにサイズ調整
        self.avoidance_dialog.set_list_sizer.Layout()
        self.avoidance_dialog.set_list_sizer.FitInside(
            self.avoidance_dialog.scrolled_window
        )
    ...
    def on_check_arm_process_alignment(self, event: wx.Event):
        ...
        if isinstance(event.GetEventObject(), wx.StaticText):
            ...
            if (
                self.arm_alignment_finger_flg_ctrl.GetValue() == 1
                or self.arm_alignment_floor_flg_ctrl.GetValue() == 1
            ):
                self.arm_process_flg_alignment.SetValue(1)

        if (
            self.arm_alignment_finger_flg_ctrl.GetValue()
            and len(self.frame.multi_panel_ctrl.file_set_list) > 0
        ):
...
class AvoidanceSet:
    def __init__(
        ...
        if file_set.is_loaded():
            ...
            for (
                rigidbody_name,
                rigidbody,
            ) in file_set.rep_model_file_ctrl.data.rigidbodies.items():
                # 処理対象剛体：有効なボーン追従剛体
                if (
                    rigidbody.isModeStatic()
                    and rigidbody.bone_index
                    in file_set.rep_model_file_ctrl.data.bone_indexes
                ):
                    self.rep_avoidances.append(
                        "{0} ({1})".format(
                            rigidbody.name,
                            file_set.rep_model_file_ctrl.data.bone_indexes[
            ...
            # 選択コントロール
            self.rep_choices = wx.ListBox(
                self.window,
                id=wx.ID_ANY,
                choices=self.rep_avoidances,
            ...
            self.copy_btn_ctrl.SetToolTip(
                "Copy contact avoidance data to the clipboard in bulk CSV format"
            )
    ...
    def on_copy(self, event: wx.Event):
        ...
        with wx.TextEntryDialog(
            self.frame,
            "Output contact avoidance data for bulk CSV.\n"
            ...
            caption="Bulk CSV Contact Avoidance Data",
            value=";".join(avoidance_txt_list),
...
class AvoidanceDialog(wx.Dialog):
    def __init__(self, parent):
        super().__init__(
            parent,
            id=wx.ID_ANY,
            title="Select Rigid Body for Avoid Contact",
            pos=(-1, -1),
            size=(800, 500),
            style=wx.DEFAULT_DIALOG_STYLE,
            name="AvoidanceDialog",
        )
        ...
        self.description_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            "You can select bone-tracking rigid bodies from the target model to avoid hand contact.\n"
            + '"Head Contact Avoidance" is a rigid body automatically calculated based on head size. If the result is not satisfactory, deselect it.\n'
            + "There are no restrictions as long as it is a bone-tracking rigid body, but if you select too many, the hand may not be able to avoid anything and unexpected results may occur.",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
