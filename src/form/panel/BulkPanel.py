...
class BulkPanel(BasePanel):
    def __init__(self, frame: wx.Frame, parent: wx.Notebook, tab_idx: int):
        ...
        self.description_txt = wx.StaticText(
            self,
            wx.ID_ANY,
            "You can specify settings in bulk and process them consecutively.",
            wx.DefaultPosition,
            wx.DefaultSize,
            0,
        )
        ...
        # バルクBULKファイルコントロール
        self.bulk_csv_file_ctrl = HistoryFilePickerCtrl(
            frame,
            self,
            "Bulk CSV",
            "Open CSV file for bulk processing",
            ("csv"),
            wx.FLP_DEFAULT_STYLE,
            "Please specify the CSV for bulk processing.\nYou can get the format from the DL button.\nYou can specify by D&D, open button, or select from history.",
            file_model_spacer=0,
            title_parts_ctrl=None,
            title_parts2_ctrl=None,
            file_histories_key="bulk_csv",
        ...
        # 一括サイジング保存ボタン
        self.save_btn_ctrl = wx.Button(
            self, wx.ID_ANY, "Save Bulk Sizing", wx.DefaultPosition, wx.Size(150, 50), 0
        )
        self.save_btn_ctrl.SetToolTip("Save the current sizing settings to CSV")
        ...
        self.check_btn_ctrl.SetToolTip("Check the settings of the specified CSV data.")
        ...
        self.bulk_btn_ctrl.SetToolTip("Execute sizing in bulk")
    ...
    def on_bulk(self, event: wx.Event):
        ...
        if self.bulk_btn_ctrl.GetLabel() == "Stop Bulk Sizing" and self.frame.worker:
            ...
            logger.warning(
                "Aborted Bulk VMD sizing process.", decoration=MLogger.DECORATION_BOX
            )
            ...
            logger.error(
                "Bulk sizing still in progress. Please finish it before running again.",
    ...
    def on_save(self, event: wx.Event):
        ...
        if not self.frame.file_panel_ctrl.file_set.motion_vmd_file_ctrl.path():
            logger.warning(
                'The "Target Motion VMD/VPD" in the file tab is blank, so the process is aborted.',
                decoration=MLogger.DECORATION_BOX,
            )
            return

        save_key = [
            "グループNo(複数人モーションは同じNo)",
            "調整対象モーションVMD/VPD(フルパス)",
            "モーション作成元モデルPMX(フルパス)",
            "モーション変換先モデルPMX(フルパス)",
            "センターXZ補正(0:無効、1:有効)",
            "上半身補正(0:無効、1:有効)",
        ...
        output_path = os.path.join(
            os.path.dirname(
                self.frame.file_panel_ctrl.file_set.motion_vmd_file_ctrl.path()
            ),
            f"BulkSizingData_{datetime.now():%Y%m%d_%H%M%S}.csv",
        )
        ...
        with open(output_path, "w", encoding="cp932", newline="") as f:
            ...
            writer.writeheader()
            writer.writerow(
                self.create_save_data(self.frame.file_panel_ctrl.file_set, 0, save_key)
            )

            for multi_idx, file_set in enumerate(
                self.frame.multi_panel_ctrl.file_set_list
            ):
                writer.writerow(
                    self.create_save_data(file_set, multi_idx + 1, save_key)
                )
        ...
        logger.info(
            "Successfully saved bulk sizing data\n\n%s",
            output_path,
            decoration=MLogger.DECORATION_BOX,
        )
        return
    ...
    def create_save_data(self, file_set: SizingFileSet, file_idx: int, save_key: list):
        ...
        save_data[save_key[4]] = (
            "1"
            if 0 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[5]] = (
            "1"
            if 1 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[6]] = (
            "1"
            if 2 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[7]] = (
            "1"
            if 3 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[8]] = (
            "1"
            if 4 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[9]] = (
            "1"
            if 5 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[10]] = (
            "1"
            if 6 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[11]] = (
            "1"
            if 7 in file_set.selected_stance_details
            and file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[12]] = (
            "1" if file_set.rep_model_file_ctrl.title_parts_ctrl.GetValue() else "0"
        )
        save_data[save_key[13]] = (
            ";".join(
                [
                    f"{om}:{rm}:{r}"
                    for (om, rm, r) in self.frame.morph_panel_ctrl.morph_set_dict[
                        file_idx
                    ].get_morph_list()
                ]
            )
            + ";"
            if file_idx in self.frame.morph_panel_ctrl.morph_set_dict
            else ""
        )
        save_data[save_key[14]] = (
            "1"
            if self.frame.arm_panel_ctrl.arm_process_flg_avoidance.GetValue()
            else "0"
        )
        save_data[save_key[15]] = (
            ";".join(list(self.frame.arm_panel_ctrl.get_avoidance_target()[file_idx]))
            + ";"
            if file_idx in self.frame.arm_panel_ctrl.get_avoidance_target()
            else ""
        )
        save_data[save_key[16]] = (
            "1"
            if self.frame.arm_panel_ctrl.arm_process_flg_alignment.GetValue()
            else "0"
        )
        save_data[save_key[17]] = (
            "1"
            if self.frame.arm_panel_ctrl.arm_alignment_finger_flg_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[18]] = (
            "1"
            if self.frame.arm_panel_ctrl.arm_alignment_floor_flg_ctrl.GetValue()
            else "0"
        )
        save_data[save_key[19]] = (
            self.frame.arm_panel_ctrl.alignment_distance_wrist_slider.GetValue()
        )
        save_data[save_key[20]] = (
            self.frame.arm_panel_ctrl.alignment_distance_finger_slider.GetValue()
        )
        save_data[save_key[21]] = (
            self.frame.arm_panel_ctrl.alignment_distance_floor_slider.GetValue()
        )
        save_data[save_key[22]] = (
            "1" if self.frame.arm_panel_ctrl.arm_check_skip_flg_ctrl.GetValue() else "0"
        )
        save_data[save_key[23]] = (
            self.frame.leg_panel_ctrl.move_correction_slider.GetValue()
        )
        save_data[save_key[24]] = (
            self.frame.leg_panel_ctrl.get_leg_offsets()[file_idx]
            if file_idx in self.frame.leg_panel_ctrl.get_leg_offsets()
            else "0"
        )
        save_data[save_key[25]] = (
            self.frame.camera_panel_ctrl.camera_vmd_file_ctrl.file_ctrl.GetPath()
        )
        save_data[save_key[26]] = (
            self.frame.camera_panel_ctrl.camera_length_slider.GetValue()
        )
        save_data[save_key[27]] = (
            self.frame.camera_panel_ctrl.camera_set_dict[
                file_idx + 1
            ].camera_model_file_ctrl.path()
            if file_idx + 1 in self.frame.camera_panel_ctrl.camera_set_dict
            else ""
        )
        save_data[save_key[28]] = (
            self.frame.camera_panel_ctrl.camera_set_dict[
                file_idx + 1
            ].camera_offset_y_ctrl.GetValue()
            if file_idx + 1 in self.frame.camera_panel_ctrl.camera_set_dict
            else ""
        )
    ...
    def check(self, event: wx.Event, is_exec: bool):
        ...
        with open(self.bulk_csv_file_ctrl.path(), encoding="cp932", mode="r") as f:
            ...
            for ridx, rows in enumerate(reader):
                ...
                arm_alignment_length_result, arm_alignment_length_datas = (
                    self.read_csv_row(
                        rows, row_no, 19, "手首の距離", False, float, None, None, None
                    )
                )
                finger_alignment_length_result, finger_alignment_length_datas = (
                    self.read_csv_row(
                        rows, row_no, 20, "指の距離", False, float, None, None, None
                    )
                )
                floor_alignment_length_result, floor_alignment_length_datas = (
                    self.read_csv_row(
                ...
                move_correction_result, move_correction_data = self.read_csv_row(
                    rows, row_no, 23, "全体移動量補正", False, float, None, None, None
                )
                leg_offset_result, leg_offset_data = self.read_csv_row(
                    rows,
                    row_no,
                    24,
                    "足ＩＫオフセット値",
                ...
                camera_y_offset_result, camera_y_offset_datas = self.read_csv_row(
                    rows, row_no, 28, "全長Yオフセット", False, float, None, None, None
                )
                ...
                result = (
                    ...
                    & org_model_result
                    & rep_model_result
                    & stance_center_xz_result
                    & stance_upper_result
                    & stance_lower_result
                    & stance_leg_ik_result
                    & stance_toe_result
                    & stance_toe_ik_result
                    & stance_shoulder_result
                    & stance_center_y_result
                    & separate_twist_result
                    & arm_check_skip_result
                    & morph_result
                    & arm_avoidance_result
                    & avoidance_name_result
                    & arm_alignment_result
                    & finger_alignment_result
                    & floor_alignment_result
                    & arm_alignment_length_result
                    & finger_alignment_length_result
                    & floor_alignment_length_result
                    & org_camera_motion_result
                    & camera_length_result
                    & org_camera_model_result
                    & camera_y_offset_result
                    & move_correction_result
                    & leg_offset_result
                )
                ...
                if result:
                    if prev_group_no != group_no[0]:
                        ...
                        service_data_txt = (
                            f"{service_data_txt}  Camera: {org_camera_motion_path}\n"
                        )
                    ...
                    service_data_txt = (
                        f"{service_data_txt}    Motion: {org_motion_path}\n"
                    )
                    service_data_txt = (
                        f"{service_data_txt}    Source Model: {org_model_path}\n"
                    )
                    service_data_txt = (
                        f"{service_data_txt}    Target Model: {rep_model_path}\n"
                    )
                    service_data_txt = (
                        f"{service_data_txt}    Leg IK Offset: {leg_offset_data}\n"
                    )
                    service_data_txt = f"{service_data_txt}    Source Camera Model: {org_camera_model_path}\n"
                    service_data_txt = (
                        f"{service_data_txt}    Y Offset: {camera_y_offset_datas}\n"
                    )
                    ...
                    service_data_txt = (
                        f"{service_data_txt}    Twist Distribution: {twist_txt}\n"
                    )
                    ...
                    service_data_txt = (
                        f"{service_data_txt}    Morph Replacement: {morph_txt}\n"
                    )
        ...
        if result:
            ...
            logger.error(
                "Aborting process as CSV data has inconsistencies.",
                decoration=MLogger.DECORATION_BOX,
            )
    ...
    def read_csv_row(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "%s row: %s (column %s) type (%s) is incorrect",
                    row_no + 1,
                    row_name,
                    row_idx + 1,
                    row_type_str,
            ...
            if rows[row_idx] and row_regex and not re.findall(row_regex, rows[row_idx]):
                logger.warning(
                    "%s row: %s (column %s) display format (%s) is incorrect",
                    row_no + 1,
                    row_name,
                    row_idx + 1,
                    row_regex_str,
            ...
            if rows[row_idx] and path_exts:
                if not rows[row_idx] or (
                    not os.path.exists(rows[row_idx])
                    or not os.path.isfile(rows[row_idx])
                ):
                    logger.warning(
                        "%s row: %s (column %s) file does not exist",
                        row_no + 1,
                        row_name,
                        row_idx + 1,
                    )
                ...
                if ext not in path_exts:
                    logger.warning(
                        "%s row: %s (column %s) file extension (%s) is incorrect",
                        row_no + 1,
                        row_name,
                        row_idx + 1,
                        ",".join(map(str, path_exts))
                        if len(path_exts) > 1
                        else path_exts,
                    )
                    return False, None
            ...
            if rows[row_idx] and row_regex:
                ...
                if row_type:
                    # 型指定がある場合は変換して返す
                    return True, [
                        row_type(v) for v in re.findall(row_regex, rows[row_idx])
                    ]
                else:
            ...
            logger.warning(
                "%s row: %s (column %s) failed to read\n%s",
                row_no + 1,
                row_name,
                row_idx + 1,
                e,
    ...
    def load(self, event, line_idx):
        ...
        with open(self.bulk_csv_file_ctrl.path(), encoding="cp932", mode="r") as f:
            ...
            for ridx, rows in enumerate(reader):
                ...
                camera_length_result, camera_length_datas = self.read_csv_row(
                    rows, row_no, 26, "距離稼働範囲", False, float, None, None, None
                )
                org_camera_model_result, org_camera_model_path = self.read_csv_row(
                    rows,
                    row_no,
                ...
                if now_motion_idx == 0:
                    ...
                    self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.title_parts_ctrl.SetValue(
                        stance_center_xz_datas[0]
                        | stance_upper_datas[0]
                        | stance_lower_datas[0]
                        | stance_leg_ik_datas[0]
                        | stance_toe_datas[0]
                        | stance_toe_ik_datas[0]
                        | stance_shoulder_datas[0]
                        | stance_center_y_datas[0]
                    )
                    ...
                    for morph_data in morph_datas:
                        m = re.findall(r"([^\:]+)\:([^\:]+)\:(\d+\.?\d*)\;", morph_data)
                        self.frame.morph_panel_ctrl.bulk_morph_set_dict[1].append(
                    ...
                    for avoidance_data in avoidance_name_datas:
                        m = re.findall(r"([^\:]+)\;", avoidance_data)
                        self.frame.arm_panel_ctrl.bulk_avoidance_set_dict[0].append(
                            m[0][0]
                        )
                    ...
                    # 移動量補正値
                    self.frame.leg_panel_ctrl.move_correction_slider.SetValue(
                        move_correction_data
                    )

                    # 足ＩＫオフセット
                    self.frame.leg_panel_ctrl.bulk_leg_offset_set_dict[0] = (
                        leg_offset_data
                    )
                    ...
                    self.frame.camera_panel_ctrl.camera_set_dict[
                        1
                    ].camera_model_file_ctrl.file_ctrl.SetPath(org_camera_model_path)
                    self.frame.camera_panel_ctrl.camera_set_dict[
                        1
                    ].camera_offset_y_ctrl.SetValue(camera_y_offset_datas)
                    ...
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].motion_vmd_file_ctrl.file_ctrl.SetPath(org_motion_path)
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].org_model_file_ctrl.file_ctrl.SetPath(org_model_path)
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].rep_model_file_ctrl.file_ctrl.SetPath(rep_model_path)
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].output_vmd_file_ctrl.file_ctrl.SetPath("")

                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].org_model_file_ctrl.title_parts_ctrl.SetValue(
                        stance_center_xz_datas[0]
                        | stance_upper_datas[0]
                        | stance_lower_datas[0]
                        | stance_leg_ik_datas[0]
                    ...
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].selected_stance_details = []
                    if stance_center_xz_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(0)
                    if stance_upper_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(1)
                    if stance_lower_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(2)
                    if stance_leg_ik_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(3)
                    if stance_toe_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(4)
                    if stance_toe_ik_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(5)
                    if stance_shoulder_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(6)
                    if stance_center_y_datas[0] == 1:
                        self.frame.multi_panel_ctrl.file_set_list[
                            now_motion_idx - 1
                        ].selected_stance_details.append(7)

                    # 捩り分散
                    self.frame.multi_panel_ctrl.file_set_list[
                        now_motion_idx - 1
                    ].rep_model_file_ctrl.title_parts_ctrl.SetValue(
                        separate_twist_datas[0]
                    )
                    ...
                    for morph_data in morph_datas:
                        ...
                        self.frame.morph_panel_ctrl.bulk_morph_set_dict[
                            now_motion_idx + 1
                        ].append((m[0][0], m[0][1], float(m[0][2])))
                    ...
                    # 足ＩＫオフセット
                    self.frame.leg_panel_ctrl.bulk_leg_offset_set_dict[
                        now_motion_idx - 1
                    ] = leg_offset_data
        ...
        if self.frame.load_worker:
            ...
            self.frame.load_worker = LoadWorkerThread(
                self.frame,
                BulkLoadThreadEvent,
                row_no if row_no > line_idx else -1,
                True,
                False,
                False,
                False,
    ...
    def on_load_result(self, event: wx.Event):
        ...
        if self.frame.worker:
            ...
            # 別スレッドで実行
            self.frame.worker = SizingWorkerThread(
                self.frame,
