...
class BaseFilePickerCtrl:
    ...
    def __init__(
        ...
        self.title_ctrl = wx.StaticText(
            parent, wx.ID_ANY, title, wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        if file_model_spacer > 0:
            self.file_model_ctrl = FileModelCtrl(
                parent, self, title, file_model_spacer, self.set_no
            )
    ...
    def on_change_file(self, event):
        ...
        if "*" in self.file_ctrl.GetPath():
            ...
            self.target_paths = [
                p for p in glob.glob(self.astr_path) if os.path.isfile(p)
            ]
        else:
            self.astr_path = None
            self.target_paths = []
    ...
    def is_valid(self):
        ...
        if self.is_aster and self.set_no <= 1:
            ...
            if os.path.exists(base_file_path):
                ...
                file_path_list = [
                    p for p in glob.glob(base_file_path) if os.path.isfile(p)
                ]
    ...
    def load(self, file_idx=0, is_check=True):
        ...
        try:
            ...
            if new_data_digest and (
                (self.data and self.data.digest != new_data_digest) or not self.data
            ):
...
class FileModelCtrl:
    def __init__(self, parent, picker, title, spacer_cnt, set_no):
        ...
        self.spacer_ctrl = wx.StaticText(
            parent, wx.ID_ANY, "".join([" " for n in range(spacer_cnt)])
        )
        ...
        self.txt_ctrl.SetBackgroundColour(
