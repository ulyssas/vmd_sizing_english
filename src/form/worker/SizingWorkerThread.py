...
class SizingWorkerThread(BaseWorkerThread):
    ...
    def thread_event(self):
        try:
            ...
            if len(now_camera_path) > 0:
                now_camera_data = (
                    self.frame.camera_panel_ctrl.camera_vmd_file_ctrl.data.copy()
                )
            ...
            if self.frame.file_panel_ctrl.file_set.is_loaded():
                proccess_key = "【No.1】{0}({1})".format(
                    os.path.basename(
                        self.frame.file_panel_ctrl.file_set.motion_vmd_file_ctrl.data.path
                    ),
                ...
                if len(now_camera_path) > 0:
                    camera_org_model = (
                        self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.data
                    )
                    if 1 in self.frame.camera_panel_ctrl.camera_set_dict:
                        if self.frame.camera_panel_ctrl.camera_set_dict[
                            1
                        ].camera_model_file_ctrl.is_set_path():
                            # カメラ元モデルが指定されている場合、カメラ元モデル再指定
                            camera_org_model = (
                                self.frame.camera_panel_ctrl.camera_set_dict[
                                    1
                                ].camera_model_file_ctrl.data
                            )
                        camera_offset_y = self.frame.camera_panel_ctrl.camera_set_dict[
                            1
                        ].camera_offset_y_ctrl.GetValue()

                morph_list, morph_seted = self.frame.morph_panel_ctrl.get_morph_list(
                    1,
                    self.frame.file_panel_ctrl.file_set.motion_vmd_file_ctrl.data.digest,
                    self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.data.digest,
                    self.frame.file_panel_ctrl.file_set.rep_model_file_ctrl.data.digest,
                )  # noqa

                if not self.frame.camera_panel_ctrl.camera_only_flg_ctrl.GetValue():
                    # 1件目のモーションとモデル
                    self.frame.file_panel_ctrl.tree_process_dict[proccess_key] = {
                        "移動縮尺補正": False
                    }

                    total_process += 2  # 基本補正・腕スタンス補正
                    if (
                        self.frame.file_panel_ctrl.file_set.org_model_file_ctrl.title_parts_ctrl.GetValue()
                        > 0
                    ):
                        total_process += len(
                            self.frame.file_panel_ctrl.file_set.get_selected_stance_details()
                        )  # スタンス追加補正
                        self.frame.file_panel_ctrl.tree_process_dict[proccess_key][
                            "スタンス追加補正"
                        ] = {}
                    ...
                    if (
                        self.frame.file_panel_ctrl.file_set.rep_model_file_ctrl.title_parts_ctrl.GetValue()
                        == 1
                    ):
                        self.frame.file_panel_ctrl.tree_process_dict[proccess_key][
                            "捩り分散"
            ...
            for multi_idx, file_set in enumerate(
                ...
                if file_set.is_loaded():
                    ...
                    # 2件目移行のモーションとモデル
                    morph_list, morph_seted = (
                        self.frame.morph_panel_ctrl.get_morph_list(
                            file_set.set_no,
                            file_set.motion_vmd_file_ctrl.data.digest,
                            file_set.org_model_file_ctrl.data.digest,
                            file_set.rep_model_file_ctrl.data.digest,
                        )
                    )  # noqa
                    ...
                    if not self.frame.camera_panel_ctrl.camera_only_flg_ctrl.GetValue():
                        ...
                        if file_set.org_model_file_ctrl.title_parts_ctrl.GetValue() > 0:
                            total_process += len(
                                file_set.get_selected_stance_details()
                            )  # スタンス追加補正
                            self.frame.file_panel_ctrl.tree_process_dict[proccess_key][
                                "スタンス追加補正"
                            ] = {}
                        ...
                        total_process += (
                            file_set.rep_model_file_ctrl.title_parts_ctrl.GetValue()
                        )  # 捩り分散
                        if (
                            file_set.rep_model_file_ctrl.title_parts_ctrl.GetValue()
                            == 1
            ...
            if (
                not self.frame.camera_panel_ctrl.camera_only_flg_ctrl.GetValue()
                and self.frame.arm_panel_ctrl.arm_process_flg_alignment.GetValue() > 0
            ):
            ...
            try:
                logger.debug(
                    "★★★result: %s, is_killed: %s", self.result, self.is_killed
                )
                if self.is_out_log or (not self.result and not self.is_killed):
                    ...
                    # 出力されたメッセージを全部出力
                    self.frame.file_panel_ctrl.console_ctrl.SaveFile(
                        filename=self.output_log_path
                    )
    ...
    def post_event(self):
        wx.PostEvent(
            self.frame,
            self.result_event(
                result=self.result and not self.is_killed,
