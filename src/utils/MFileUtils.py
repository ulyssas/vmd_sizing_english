...
def get_output_morph_path(base_file_path: str, org_pmx_path: str, rep_pmx_path: str):
    ...
    if (
        ...
        or not os.path.exists(rep_pmx_path)
    ):
        return ""
    ...
    motion_vmd_file_name, motion_vmd_ext = os.path.splitext(
        os.path.basename(file_path_list[0])
    )
    # 作成元モデルファイル名・拡張子
    ...
    # 出力ファイルパス生成
    new_output_morph_path = os.path.join(
        motion_vmd_dir_path,
        "{0}_{1}_{2}{3}".format(
            motion_vmd_file_name, org_pmx_file_name, rep_pmx_file_name, ".csv"
        ),
    )
...
def get_output_vmd_path(
    base_file_path: str,
    rep_pmx_path: str,
    detail_stance_flg: bool,
    twist_flg: bool,
    arm_process_flg_avoidance: bool,
    arm_process_flg_alignment: bool,
    is_morphs: bool,
    output_vmd_path: str,
    is_force=False,
):
    ...
    # 出力ファイルパス生成
    new_output_vmd_path = os.path.join(
        motion_vmd_dir_path,
        "{0}_{1}{2}_{3:%Y%m%d_%H%M%S}{4}".format(
            motion_vmd_file_name, rep_pmx_file_name, suffix, datetime.now(), ".vmd"
        ),
    )

    # ファイルパス自体が変更されたか、自動生成ルールに則っている場合、ファイルパス変更
    if is_force or is_auto_vmd_output_path(
        output_vmd_path,
        motion_vmd_dir_path,
        motion_vmd_file_name,
        ".vmd",
        rep_pmx_file_name,
    ):
        try:
...
# 自動生成ルールに則ったパスか
def is_auto_vmd_output_path(
    output_vmd_path: str,
    motion_vmd_dir_path: str,
    motion_vmd_file_name: str,
    motion_vmd_ext: str,
    ...
    escaped_motion_vmd_file_name = escape_filepath(
        os.path.join(motion_vmd_dir_path, motion_vmd_file_name)
    )
    escaped_rep_pmx_file_name = escape_filepath(rep_pmx_file_name)
    escaped_motion_vmd_ext = escape_filepath(motion_vmd_ext)

    new_output_vmd_pattern = re.compile(
        r"^%s_%s%s%s$"
        % (
            escaped_motion_vmd_file_name,
            escaped_rep_pmx_file_name,
            r"_?\w*_\d{8}_\d{6}",
            escaped_motion_vmd_ext,
        )
    )
...
def get_output_camera_vmd_path(
    base_file_path: str,
    rep_pmx_path: str,
    output_camera_vmd_path: str,
    camera_length: float,
    is_force=False,
):
    ...
    motion_camera_vmd_file_name, motion_camera_vmd_ext = os.path.splitext(
    ...
    # 出力ファイルパス生成
    new_output_camera_vmd_path = os.path.join(
        motion_camera_vmd_dir_path,
        "{0}_{1}({2})_{3:%Y%m%d_%H%M%S}{4}".format(
            motion_camera_vmd_file_name,
            rep_pmx_file_name,
            camera_length,
            datetime.now(),
            ".vmd",
    ...
    # ファイルパス自体が変更されたか、自動生成ルールに則っている場合、ファイルパス変更
    if is_force or is_auto_camera_vmd_output_path(
...
# 自動生成ルールに則ったパスか
def is_auto_camera_vmd_output_path(
    output_camera_vmd_path: str,
    motion_camera_vmd_dir_path: str,
    motion_camera_vmd_file_name: str,
    motion_camera_vmd_ext: str,
    rep_pmx_file_name: str,
):
    ...
    # 新しく設定しようとしている出力ファイルパスの正規表現
    escaped_motion_camera_vmd_file_name = escape_filepath(
        os.path.join(motion_camera_vmd_dir_path, motion_camera_vmd_file_name)
    )
    escaped_rep_pmx_file_name = escape_filepath(rep_pmx_file_name)
    escaped_motion_camera_vmd_ext = escape_filepath(motion_camera_vmd_ext)

    new_output_camera_vmd_pattern = re.compile(
        r"^%s_%s(\d+)_%s%s$"
        % (
            escaped_motion_camera_vmd_file_name,
            escaped_rep_pmx_file_name,
            r"_\d{8}_\d{6}",
            escaped_motion_camera_vmd_ext,
        )
    )
