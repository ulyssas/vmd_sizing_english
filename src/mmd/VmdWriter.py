    ...
    def write(self):
        ...
        if len(bone_frames) > 0 or len(morph_frames) > 0:
            try:
                # モデル名を20byteで切る
                model_bname = (
                    self.data_set.rep_model.name.encode("cp932")
                    .decode("shift_jis")
                    .encode("shift_jis")[:20]
                )
            except Exception:
        ...
        for cf in camera_frames:
            cf.write(fout)
        fout.write(
            struct.pack("<L", len(self.data_set.motion.lights))
        )  # 照明キーフレーム数
        for cf in self.data_set.motion.lights:
            cf.write(fout)
        fout.write(
            struct.pack("<L", len(self.data_set.motion.shadows))
        )  # セルフ影キーフレーム数
        ...
        if len(camera_frames) == 0:
            fout.write(
                struct.pack("<L", len(self.data_set.motion.showiks))
