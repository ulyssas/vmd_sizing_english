    ...
    for base_file_name in ["揺れる紙A4", "揺れる紙写真"]:
        for prefix, max_cnt in [("3x3", 2), ("5x5", 4), ("7x7", 6), ("9x9", 8)]:
            for physics in [None, "横一列", "縦一列", "一点", "フル"]:
                ...
                model = PmxReader(
                    f"D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Accessory\\文房具\\揺れる紙 miu\\{base_file_name}.pmx",
                    is_check=False,
                    is_sizing=False,
                ).read_data()
                ...
                model.bones["全ての親"].position = MVector3D(
                    min_vec.x(), 0, max_vec.z()
                )
                ...
                edge_display_name = "隅"
                model.display_slots[edge_display_name] = DisplaySlot(
                    edge_display_name, edge_display_name, 0, 0
                )

                display_name = "紙回転"
                model.display_slots[display_name] = DisplaySlot(
                    display_name, display_name, 0, 0
                )

                bone_xs = np.append(
                    np.arange(min_vec.x(), max_vec.x(), (size.x() / max_cnt)),
                    max_vec.x(),
                )
                bone_zs = np.append(
                    np.arange(max_vec.z(), min_vec.z(), -(size.z() / max_cnt)),
                    min_vec.z(),
                )

                r_size = MVector3D(
                    abs(bone_xs[1] - bone_xs[0]), 0, abs(bone_zs[1] - bone_zs[0])
                )
                ...
                for zidx in range(max_cnt + 1):
                    for xidx in range(max_cnt + 1):
                        ...
                        if (
                            (physics == "一点" and xidx in [0] and zidx in [0])
                            or (
                                physics in ["横一列", "一点"]
                                and xidx in [0, center_cnt, max_cnt]
                                and zidx in [0]
                            )
                            or (
                                physics == "縦一列"
                                and xidx in [0]
                                and zidx in [0, center_cnt, max_cnt]
                            )
                        ):
                            ...
                            # 表示枠
                            model.display_slots[edge_display_name].references.append(
                                edge_bone.index
                            )
                ...
                if physics:
                            ...
                            for zidx in range(max_cnt + 1):
                                ...
                                for target_name in [
                                    ...
                                    if target_name in model.bones:
                                        target_poses.append(
                                            model.bones[target_name].position.data()
                                        )
                                center_pos = MVector3D(
                                    np.mean(target_poses, axis=0)
                                ) + MVector3D(-r_size.x() / 2, 0, r_size.z() / 2)
                                ...
                                mode = 1
                                if (
                                    (physics in "横一列" and zidx == 0)
                                    or (physics == "縦一列" and xidx == 0)
                                    or (physics == "一点" and zidx == 0 and xidx == 0)
                                ):
                                    mode = 0
                                rigidbody = RigidBody(
                                    bone.name,
                                    ...
                                    1,
                                    MVector3D(abs(r_size.x() / 2), 0.1, r_size.z() / 2),
                                    center_pos,
                                    MVector3D(),
                                    0.5,
                                    0.5,
                    ...
                    if physics in ["横一列", "一点", "フル"]:
                            ...
                            for xidx in range(max_cnt + 1):
                                ...
                                if (
                                    ...
                                    joint = Joint(
                                        ...
                                        - MVector3D(0, 0, r_size.z() / 2),
                                        MVector3D(),
                                        MVector3D(),
                                        MVector3D(),
                                        MVector3D(
                                            math.radians(-(max_cnt * 5)),
                                            math.radians(0),
                                            math.radians(0),
                                        ),
                                        MVector3D(
                                            math.radians((max_cnt * 3)),
                                            math.radians(0),
                                            math.radians(0),
                                        ),
                                        MVector3D(100, 100, 100),
                                        MVector3D(100, 100, 100),
                                    )
                        ...
                        for zidx in range(max_cnt + 1):
                            for xidx in range(max_cnt + 1):
                                right_bottom_joint_name = (
                                    f"紙-{(xidx + 1):02d}-{(zidx + 1):02d}"
                                )
                                bone_name = f"紙-{(xidx):02d}-{(zidx):02d}"
                                if (
                                    right_bottom_joint_name in model.rigidbodies
                                    and bone_name in model.rigidbodies
                                ):
                                    joint = Joint(
                                        f"{bone_name}_右",
                        ...
                        for zidx in range(max_cnt + 1):
                            for xidx in range(1, max_cnt + 1):
                                left_bottom_joint_name = (
                                    f"紙-{(xidx - 1):02d}-{(zidx + 1):02d}"
                                )
                                bone_name = f"紙-{(xidx):02d}-{(zidx):02d}"
                                if (
                                    left_bottom_joint_name in model.rigidbodies
                                    and bone_name in model.rigidbodies
                                ):
                                    joint = Joint(
                                        f"{bone_name}_左",
                                        ...
                                        0,
                                        model.rigidbodies[bone_name].index,
                                        model.rigidbodies[left_bottom_joint_name].index,
                                        model.bones[bone_name].position
                                        + MVector3D(r_size.x() / 2, 0, 0),
                                        MVector3D(),
                                        MVector3D(),
                        ...
                        for zidx in range(start_y, max_cnt + 1):
                            for xidx in range(max_cnt + 1):
                                horizonal_joint_name = (
                                    f"紙-{(xidx + 1):02d}-{(zidx):02d}"
                                )
                                bone_name = f"紙-{(xidx):02d}-{(zidx):02d}"
                                if (
                                    horizonal_joint_name in model.rigidbodies
                                    and bone_name in model.rigidbodies
                                ):
                                    joint = Joint(
                                        f"{bone_name}_横",
                                        ...
                                        0,
                                        model.rigidbodies[bone_name].index,
                                        model.rigidbodies[horizonal_joint_name].index,
                                        model.bones[bone_name].position
                                        + MVector3D(r_size.x() / 2, 0, 0),
                                        MVector3D(),
                                        MVector3D(),
                ...
                for xidx, (k, vs) in enumerate(model.vertices.items()):
                    for zidx, v in enumerate(vs):
                        ...
                        r_min_vec = MVector3D(
                            list(target_xs.keys())[0], 0, list(target_zs.keys())[-1]
                        )
                        r_max_vec = MVector3D(
                            list(target_xs.keys())[-1], 0, list(target_zs.keys())[0]
                        )

                        left_top_weight = max(
                            0,
                            (
                                (r_size.x() - (v.position.x() - r_min_vec.x()))
                                / r_size.x()
                            )
                            * ((v.position.z() - r_min_vec.z()) / r_size.z()),
                        )
                        left_bottom_weight = max(
                            ...
                            * (r_size.z() - (v.position.z() - r_min_vec.z()))
                            / r_size.z(),
                        )
                        ...
                        if np.count_nonzero(weight_values) == 1:
                            ...
                            v.deform = Bdef4(
                                model.bones[left_top_name].index,
                                model.bones[right_top_name].index,
                                model.bones[left_bottom_name].index,
                                model.bones[right_bottom_name].index,
                                weight_values[0],
                                weight_values[1],
                                weight_values[2],
                                weight_values[3],
