...
def exec():
    model = PmxReader(
        "D:\\MMD\\Blender\\スカート\\cloak.pmx", is_check=False, is_sizing=False
    ).read_data()
    ...
    for (
        ...
        collision_group_idx,
        rigidbody_param_to,
        min_vertical_limit_rot,
        max_vertical_limit_rot,
        min_horizonal_limit_rot,
        max_horizonal_limit_rot,
    ) in [
        (
            "mt",
            "マント",
            ...
            2,
            1,
            RigidBodyParam(0.2, 0.99, 0.99, 0, 0.5),
            15,
            30,
            20,
            40,
        ...
        root_bone = Bone(
            f"{target_bone_name}中心",
            f"{target_bone_name}中心",
            model.bones[parent_bone_name].position,
            model.bones[parent_bone_name].index,
            0,
        ...
        model.display_slots[target_bone_name].references.append(
            (0, model.bones[root_bone.name].index)
        )
        ...
        if tailor_type == 1:
            ...
            root_right_bone = Bone(
                root_right_bone_name,
                root_right_bone_name,
                model.bones[root_bone.name].position,
                model.bones["右肩"].index,
                0,
                0x0000 | 0x0002 | 0x0008 | 0x0010,
            )
            ...
            model.display_slots[target_bone_name].references.append(
                (0, model.bones[root_right_bone.name].index)
            )
            model.display_slots[target_bone_name].references.append(
                (0, model.bones[root_left_bone.name].index)
            )
        ...
        for ik, index_idx in enumerate(model.material_indices[target_material_name]):
            ...
            diagonal_idx = np.argmax(
                [
                    MQuaternion.rotationTo(v02_diff, v12_diff).toDegree(),
                    MQuaternion.rotationTo(v10_diff, v20_diff).toDegree(),
                    MQuaternion.rotationTo(v01_diff, v21_diff).toDegree(),
                ]
            )  # noqa
            ...
            y_idx = np.argmin(
                np.abs(
                    [
                        MVector3D.dotProduct(v10_diff, v20_diff),
                        MVector3D.dotProduct(v01_diff, v21_diff),
                        MVector3D.dotProduct(v02_diff, v12_diff),
                    ]
                )
            )  # noqa
            # 残りが水平方向と見なす
            ...
            if len(duplicate_idxs) > 1:
                for i0 in duplicate_idxs:
                    for i1 in duplicate_idxs:
                        if i0 != i1:
                            if (
                                model.vertex_dict[i1]
                                not in indices_dict[i0]["duplicate"]
                            ):
                                indices_dict[i0]["duplicate"].append(
                                    model.vertex_dict[i1]
                                )
                            if (
                                model.vertex_dict[i0]
                                not in indices_dict[i1]["duplicate"]
                            ):
                                indices_dict[i1]["duplicate"].append(
                                    model.vertex_dict[i0]
                                )
        ...
        if not start_vertex_idx:
            ...
            start_vertex_idx = [
                ...
                if model.vertex_dict[i].position
                == MVector3D(sorted_top_vertex_poses[0])
            ][0]
        ...
        if (
            len(indices_dict[target_y_idx]["y-"]) == 0
            and len(indices_dict[target_y_idx]["duplicate"]) > 0
        ):
        ...
        # Y軸に等間隔にボーンを配置する
        for yidx, now_y in enumerate(
            [start_ys[0]]
            + np.linspace(start_ys[1], start_ys[-1], max_y_cnt - 1).tolist()
        ):
            # 等分割したYに最も近い頂点のY値
            ...
            if is_circles[yidx]:
                # 円周の場合
                target_x_idxs = (
                    x_idxs[:-1] if is_full_regist else x_idxs[:] + [x_idxs[0]]
                )
            else:
                # 一枚の場合
            ...
            for xidx, now_x_idx in enumerate(target_x_idxs):
                ...
                bone_vertex_map[bone_name] = {
                    "xidx": mesh_xidx,
                    "yidx": mesh_yidx,
                    "vertex_idxs": [now_x_idx]
                    + [i.index for i in indices_dict[now_x_idx]["duplicate"]],
                }
        ...
        end_right_top_bone_name = (
            f"{target_bone_name}-{(max_y_cnt - 1):03d}-{(1 + x_density):03d}"
        )
        end_right_bottom_bone_name = (
            f"{target_bone_name}-{(max_y_cnt):03d}-{(1 + x_density):03d}"
        )

        # 末端表示枠の最大値
        end_length = np.mean(
            [
                target_bones[end_left_top_bone_name].position.distanceToPoint(
                    target_bones[end_right_top_bone_name].position
                ),
                target_bones[end_left_bottom_bone_name].position.distanceToPoint(
                    target_bones[end_right_bottom_bone_name].position
                ),
            ]
        )
        ...
        for yidx in range(1, max_y_cnt + 1):
            for xidx in range(0, max_x_cnt + 1):
                ...
                # 親ボーン名
                parent_bone_name = (
                    f"{target_bone_name}-{(yidx - 1):03d}-{(xidx + 1):03d}"
                )
                ...
                if xidx == 0:
                    if parent_bone_name not in model.bones:
                        if tailor_type == 1 and root_right_bone and root_left_bone:
                            for direction in ["右", "左"]:
                                ...
                                distance = (
                                    arm_bone.position.distanceToPoint(
                                        elbow_bone.position
                                    )
                                    / 2
                                )
                                ...
                                if (
                                    bone.position.distanceToPoint(
                                        shoulder_bone.position
                                    )
                                    <= distance
                                ):
                                    # 肩に近い場合、左右ボーンに割り振る
                    ...
                    if yidx == 1:
                        ...
                        parent_bone_name = (
                            f"{target_bone_name}-{(yidx - 1):03d}-{(max_x_cnt + 1):03d}"
                        )
                    is_regist = True
                else:
                    # 指定された頂点から横方向のINDEXリストを取得
                    ...
                    # 隣ボーンとの距離
                    prev_vidxs = x_vertex_idxs[
                        (bone_vertex_map[prev_bone_name]["xidx"]) : (xidx + 1)
                    ]
                    next_vidxs = x_vertex_idxs[(xidx):]

                    for pvidx, nvidx in zip(prev_vidxs[:-1], prev_vidxs[1:]):
                        prev_brother_lengths.append(
                            model.vertex_dict[pvidx].position.distanceToPoint(
                                model.vertex_dict[nvidx].position
                            )
                        )

                    for pvidx, nvidx in zip(next_vidxs[:-1], next_vidxs[1:]):
                        next_brother_lengths.append(
                            model.vertex_dict[pvidx].position.distanceToPoint(
                                model.vertex_dict[nvidx].position
                            )
                        )
                    ...
                    if (is_full_regist and xidx % x_density == 0) or (
                        end_length * 0.8 < prev_brother_length
                        and end_length * 0.8 < next_brother_length
                    ):  # noqa
                        ...
                        if yidx == 1:
                            ...
                            byidx = yidx - 1
                            parent_bone_name = (
                                f"{target_bone_name}-{(byidx):03d}-{(xidx + 1):03d}"
                            )
                            ...
                            if not is_parent:
                                for bxidx in range(
                                    xidx, max(0, int(prev_bone_name.split("-")[-1])), -1
                                ):
                            ...
                            if not is_parent:
                                if is_circles[yidx - 1]:
                                    # 最後までいって見つからなければ最初
                                    parent_bone_name = (
                                        f"{target_bone_name}-{(yidx - 1):03d}-{(1):03d}"
                                    )
                                else:
                                    # 一枚物の場合、最後
        ...
        for parent_bone_name, bone_names in parent_bones.items():
            ...
            sorted_bone_names = sorted(
                bone_names,
                key=lambda x: [
                    abs(int(parent_bone_name.split("-")[2]) - int(x.split("-")[2])),
                    -int(x.split("-")[1]),
                ],
            )
        ...
        for ridx, row_range in enumerate(vertices_map):
            for widx, wvidx in enumerate(row_range):
                if widx > 0:
                    vertex_horizonal_lengths[ridx, widx] = model.vertex_dict[
                        vertices_map[ridx, widx]
                    ].position.distanceToPoint(
                        model.vertex_dict[vertices_map[ridx, widx - 1]].position
                    )
                if ridx > 0:
                    vertex_vertical_lengths[ridx, widx] = model.vertex_dict[
                        vertices_map[ridx, widx]
                    ].position.distanceToPoint(
                        model.vertex_dict[vertices_map[ridx - 1, widx]].position
                    )
        ...
        while yidx > 1:
            ...
            right_bottom_bone_name = model.bone_indexes[
                model.bones[left_bottom_bone_name].index + 1
            ]
            left_top_bone_name = model.bone_indexes[
                model.bones[left_bottom_bone_name].parent_index
            ]
            right_top_bone_name = model.bone_indexes[
                model.bones[right_bottom_bone_name].parent_index
            ]
            ...
            while xidx <= max_x_cnt:
                ...
                if int(left_bottom_bone_name.split("-")[1]) == max_y_cnt:
                    if int(right_bottom_bone_name.split("-")[2]) == 1:
                        ...
                        vertices_range = vertices_map[
                            left_top_bone_map["yidx"] :, left_top_bone_map["xidx"] :
                        ]
                        vertex_horizonal_length_range = vertex_horizonal_lengths[
                            left_top_bone_map["yidx"] :, left_top_bone_map["xidx"] :
                        ]
                        vertex_vertical_length_range = vertex_vertical_lengths[
                            left_top_bone_map["yidx"] :, left_top_bone_map["xidx"] :
                        ]
                    else:
                        # 全部がある場合はそこまで
                ...
                elif is_circles[yidx - 1] and (
                    int(right_top_bone_name.split("-")[2]) == 1
                    or int(right_bottom_bone_name.split("-")[2]) == 1
                ):
                    # 最後まで
                    max_xidx = 0
                    ...
                    # 右端がない場合、最後まで
                    vertices_range = vertices_map[
                        left_top_bone_map["yidx"] : (left_bottom_bone_map["yidx"] + 1),
                        left_top_bone_map["xidx"] :,
                    ]
                    vertex_horizonal_length_range = vertex_horizonal_lengths[
                        left_top_bone_map["yidx"] : (left_bottom_bone_map["yidx"] + 1),
                ...
                for hi, h_vertices in enumerate(vertices_range):
                    for vi, vertex_idx in enumerate(h_vertices):
                        ...
                        v_horizonal_length = np.sum(
                            vertex_horizonal_length_range[hi, : (vi + 1)]
                        )
                        vertical_length = np.sum(vertex_vertical_length_range[:, vi])
                        v_vertical_length = np.sum(
                            vertex_vertical_length_range[: (hi + 1), vi]
                        )

                        left_top_weight = max(
                            0,
                            ((horizonal_length - v_horizonal_length) / horizonal_length)
                            * ((vertical_length - v_vertical_length) / vertical_length),
                        )  # * 1.9
                        left_bottom_weight = max(
                            0,
                            ((horizonal_length - v_horizonal_length) / horizonal_length)
                            * ((v_vertical_length) / vertical_length),
                        )  # * 0.1
                        right_top_weight = max(
                            0,
                            ((v_horizonal_length) / horizonal_length)
                            * ((vertical_length - v_vertical_length) / vertical_length),
                        )  # * 1.9
                        ...
                        if int(left_bottom_bone_name.split("-")[1]) == max_y_cnt:
                            ...
                            weight_names = np.array(
                                [left_top_bone_name, right_top_bone_name]
                            )
                            # ウェイト
                            total_weights = np.array(
                                [
                                    left_top_weight + left_bottom_weight,
                                    right_top_weight + right_bottom_weight,
                                ]
                            )
                        else:
                            # 処理対象全ボーン名
                        ...
                        if len(np.nonzero(total_weights)[0]) > 0:
                            weights = total_weights / total_weights.sum(
                                axis=0, keepdims=1
                            )
                                ...
                                if not vv.deform:
                                    if np.count_nonzero(weights) == 1:
                                        ...
                                        vv.deform = Bdef4(
                                            ...
                                            model.bones[
                                                weight_names[weight_idxs[-3]]
                                            ].index,
                                            model.bones[
                                                weight_names[weight_idxs[-4]]
                                            ].index,
                                            weights[weight_idxs[-1]],
                ...
                if left_top_bone_name not in created_rigidbodies:
                    ...
                    # 剛体計算用ボーン
                    rigidbody_xis = np.array(
                        [
                            int(left_top_bone_name.split("-")[-1]),
                            int(left_bottom_bone_name.split("-")[-1]),
                            int(right_top_bone_name.split("-")[-1]),
                            int(right_bottom_bone_name.split("-")[-1]),
                            int(
                                model.bone_indexes[
                                    model.bones[left_top_bone_name].index + 1
                                ].split("-")[-1]
                            ),
                        ]
                    )
                    ...
                    if is_circles[yidx - 1] and (
                        int(right_top_bone_name.split("-")[-1]) == 1
                        or int(right_bottom_bone_name.split("-")[-1]) == 1
                    ):
                        rigidbody_right_top_bone_name = (
                            f"{target_bone_name}-{(yi):03d}-{(1):03d}"
                        )
                    else:
                    ...
                    if is_circles[yidx - 1] and (
                        int(right_bottom_bone_name.split("-")[-1]) == 1
                        or int(right_bottom_bone_name.split("-")[-1]) == 1
                    ):
                        rigidbody_right_bottom_bone_name = (
                            f"{target_bone_name}-{(yi + 1):03d}-{(1):03d}"
                        )
                    else:
                    ...
                    if int(rigidbody_right_top_bone_name.split("-")[-1]) == 1:
                        slope_xidx = int(
                            int(rigidbody_left_top_bone_name.split("-")[-1])
                            + (
                                (
                                    (end_x_idxs[-1] + 2)
                                    - int(rigidbody_left_top_bone_name.split("-")[-1])
                        ...
                        slope_xidx = int(
                            ...
                            + (
                                (
                                    int(rigidbody_right_top_bone_name.split("-")[-1])
                                    - int(rigidbody_left_top_bone_name.split("-")[-1])
                                )
                                / 2
                            )
                        )  # noqa
                    left_above_bone_name = (
                        f"{target_bone_name}-{(yi):03d}-{slope_xidx:03d}"
                    )
                    left_below_bone_name = (
                        f"{target_bone_name}-{(yi + 1):03d}-{slope_xidx:03d}"
                    )
                    shape_rotation_radians = MVector3D()
                    shape_axis = (
                        target_bones[left_below_bone_name].position
                        - target_bones[left_above_bone_name].position
                    ).normalized()
                    shape_axis_up = MVector3D(shape_axis.x(), 0, shape_axis.z())
                    shape_axis_cross = MVector3D.crossProduct(
                        shape_axis, shape_axis_up
                    ).normalized()
                    ...
                    # else:
                    shape_rotation_qq = MQuaternion.fromDirection(
                        shape_axis, shape_axis_cross
                    )
                    ...
                    shape_rotation_radians = MVector3D(
                        math.radians(shape_rotation_euler.x()),
                        math.radians(shape_rotation_euler.y()),
                        math.radians(shape_rotation_euler.z()),
                    )
                    ...
                    x_size = np.max(
                        [
                            target_bones[
                                ...
                                target_bones[rigidbody_left_top_bone_name].position
                            ),
                            target_bones[
                                rigidbody_right_bottom_bone_name
                            ].position.distanceToPoint(
                                target_bones[rigidbody_left_bottom_bone_name].position
                            ),
                        ]
                    )
                    y_size = np.max(
                        [
                            target_bones[
                                ...
                                target_bones[rigidbody_right_bottom_bone_name].position
                            ),
                            target_bones[
                                rigidbody_left_top_bone_name
                            ].position.distanceToPoint(
                                target_bones[rigidbody_left_bottom_bone_name].position
                            ),
                        ]
                ...
                if len(model.bones) == model.bones[right_bottom_bone_name].index + 1:
                    # 最後の場合クリア
                    right_bottom_bone_name = (
                        f"{target_bone_name}-{(yidx):03d}-{(1):03d}"
                    )
                else:
                    right_bottom_bone_name = model.bone_indexes[
                        model.bones[right_bottom_bone_name].index + 1
                    ]
                    if (
                        left_bottom_bone_name.split("-")[1]
                        != right_bottom_bone_name.split("-")[1]
                    ):
                        # 段が違う場合クリア
                ...
                logger.debug(
                    f"({xidx}:{yidx}) {left_bottom_bone_name}, {right_bottom_bone_name}, {left_top_bone_name}, {right_top_bone_name}"
                )
        ...
        vertical_limit_min_rots = np.linspace(
            -min_vertical_limit_rot, -max_vertical_limit_rot, max_y_cnt - 1
        )
        vertical_limit_max_rots = np.linspace(
            min_vertical_limit_rot, max_vertical_limit_rot, max_y_cnt - 1
        )
        horizonal_limit_min_rots = np.linspace(
            -min_horizonal_limit_rot, -max_horizonal_limit_rot, max_x_cnt
        )
        horizonal_limit_max_rots = np.linspace(
            min_horizonal_limit_rot, max_horizonal_limit_rot, max_x_cnt
        )
        ...
        while yidx > 1:
            ...
            while (xidx <= max_x_cnt and is_circles[yidx - 1]) or (
                xidx < max_x_cnt and not is_circles[yidx - 1]
            ):
                target_vertex_joints = [
                    (left_top_bone_name, left_bottom_bone_name, right_bottom_bone_name)
                ]
                ...
                for (
                    joint_src_bone_name,
                    joint_dest_bone_name,
                    joint_horizonal_name,
                ) in target_vertex_joints:  # noqa
                    # 縦ジョイント
                    joint_name = f"↓|{joint_src_bone_name}|{joint_dest_bone_name}"

                    if (
                        joint_name in created_joints
                        or joint_src_bone_name not in model.rigidbodies
                        or joint_dest_bone_name not in model.rigidbodies
                    ):
                        # 登録済みはスルー
                        continue
                    ...
                    # 縦ジョイント
                    joint_horizonal_vec = (
                        joint_horizonal_bone.position - joint_dest_bone.position
                    ) / 2
                    joint_vec = joint_dest_bone.position + MVector3D(
                        joint_horizonal_vec.x(), 0, joint_horizonal_vec.z()
                    )

                    # 回転量
                    joint_src_radians = created_rigidbodies[
                        joint_src_bone.name
                    ].shape_rotation
                    joint_dest_radians = created_rigidbodies[
                        joint_dest_bone.name
                    ].shape_rotation
                    joint_src_qq = MQuaternion.fromEulerAngles(
                        math.degrees(joint_src_radians.x()),
                        math.degrees(joint_src_radians.y()),
                        math.degrees(joint_src_radians.z()),
                    )
                    joint_dest_qq = MQuaternion.fromEulerAngles(
                        math.degrees(joint_dest_radians.x()),
                        math.degrees(joint_dest_radians.y()),
                        math.degrees(joint_dest_radians.z()),
                    )
                    joint_euler = MQuaternion.slerp(
                        joint_src_qq, joint_dest_qq, 0.5
                    ).toEulerAngles()
                    joint_radians = MVector3D(
                        math.radians(joint_euler.x()),
                        math.radians(joint_euler.y()),
                        math.radians(joint_euler.z()),
                    )
                    ...
                    joint = Joint(
                        ...
                        joint_name,
                        0,
                        model.rigidbodies[joint_src_bone.name].index,
                        model.rigidbodies[joint_dest_bone.name].index,
                        joint_vec,
                        joint_radians,
                        MVector3D(
                            vertical_limit_min_movs[yi],
                            vertical_limit_min_movs[yi],
                            vertical_limit_min_movs[yi],
                        ),
                        MVector3D(
                            vertical_limit_max_movs[yi],
                            vertical_limit_max_movs[yi],
                            vertical_limit_max_movs[yi],
                        ),
                        MVector3D(
                            math.radians(vertical_limit_min_rots[yi]),
                            math.radians(vertical_limit_min_rots[yi]),
                            math.radians(vertical_limit_min_rots[yi]),
                        ),
                        MVector3D(
                            math.radians(vertical_limit_max_rots[yi]),
                            math.radians(vertical_limit_max_rots[yi]),
                            math.radians(vertical_limit_max_rots[yi]),
                        ),
                        MVector3D(),
                ...
                for (
                    joint_src_bone_name,
                    joint_dest_bone_name,
                    joint_bottom_bone_name,
                ) in [
                    (left_top_bone_name, right_top_bone_name, left_bottom_bone_name),
                    (left_bottom_bone_name, right_bottom_bone_name, left_top_bone_name),
                ]:  # noqa
                    # 横ジョイント
                    joint_name = f"→|{joint_src_bone_name}|{joint_dest_bone_name}"
                    ...
                    joint_vertical_vec = (
                        joint_bottom_bone.position - joint_src_bone.position
                    ) / 2
                    joint_center_vec = (
                        joint_dest_bone.position - joint_src_bone.position
                    ) / 2
                    joint_vec = joint_dest_bone.position + MVector3D(
                        joint_vertical_vec.x(),
                        joint_vertical_vec.y() + joint_center_vec.y(),
                        joint_vertical_vec.z(),
                    )
                    ...
                    joint = Joint(
                        ...
                        joint_radians,
                        MVector3D(
                            horizonal_limit_min_movs[xi],
                            horizonal_limit_min_movs[xi],
                            horizonal_limit_min_movs[xi],
                        ),
                        MVector3D(
                            horizonal_limit_max_movs[xi],
                            horizonal_limit_max_movs[xi],
                            horizonal_limit_max_movs[xi],
                        ),
                        MVector3D(
                            math.radians(horizonal_limit_min_rots[xi]),
                            math.radians(horizonal_limit_min_rots[xi]),
                            math.radians(horizonal_limit_min_rots[xi]),
                        ),
                        MVector3D(
                            math.radians(horizonal_limit_max_rots[xi]),
                            math.radians(horizonal_limit_max_rots[xi]),
                            math.radians(horizonal_limit_max_rots[xi]),
                        ),
                        MVector3D(),
        ...
        model.comment += "\r\n　　末端剛体: 質量: {0}, 移動減衰: {1}, 回転減衰: {2}, 反発力: {3}, 摩擦力: {4}".format(
            round(rigidbody_param_to.mass),
            round(rigidbody_param_to.linear_damping, 6),
            round(rigidbody_param_to.angular_damping, 6),
            round(rigidbody_param_to.restitution, 6),
            round(rigidbody_param_to.friction, 6),
        )
    ...
    new_file_path = os.path.join(
        ...
        "{0}_{1:%Y%m%d_%H%M%S}{2}".format(
            os.path.basename(model.path.split(".")[0]), datetime.now(), ".pmx"
        ),
    )
...
def get_edge_bone_names(
    ...
    org_left_bottom_bone_name = (
        f"{target_bone_name}-{(yidx2 + 1):03d}-{(xidx1 + 1):03d}"
    )
    org_right_top_bone_name = f"{target_bone_name}-{(yidx1 + 1):03d}-{(xidx2 + 1):03d}"
    org_right_bottom_bone_name = (
        f"{target_bone_name}-{(yidx2 + 1):03d}-{(xidx2 + 1):03d}"
    )
    ...
    if org_left_top_bone_name in model.bones:
        ...
        for bxidx in range(xidx1, -1, -1):
            ...
            if brother_bone_name not in model.bones:
                ...
                org_left_bottom_bone_name = (
                    f"{target_bone_name}-{(yidx2 + 1):03d}-{(bxidx):03d}"
                )
                org_right_top_bone_name = (
                    f"{target_bone_name}-{(yidx1 + 1):03d}-{(bxidx + 1):03d}"
                )
                break
        ...
        if len(next_xs) == 0:
            ...
            if len(indices_dict[target_x_idx]["duplicate"]) > 0:
                next_xs = [
                    ...
                    if v.index not in x_idxs
                    and v.index not in duplicate_idxs
                    and len(indices_dict[v.index]["x"]) > 0
                ]
                if len(next_xs) > 0:
                    next_x_v = next_xs[0]
