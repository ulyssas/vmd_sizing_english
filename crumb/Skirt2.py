...
def exec():
    ...
    model = PmxReader(
        "D:\\MMD\\Blender\\スカート\\double09.pmx", is_check=False, is_sizing=False
    ).read_data()
    ...
    for (
        ...
        ("内sk", "内スカート", 10, 3, 2, RigidBodyParam(1, 0.9999, 0.9999, 0, 0.5)),
        ("外sk", "外スカート", 12, 3, 3, RigidBodyParam(0.5, 0.9999, 0.9999, 0, 0.5)),
    ]:
        # 表示枠定義
        ...
        for yidx in range(1, max_y_cnt + 1):
            for xidx in range(0, max_x_idx + 1):
                ...
                # 表示先ボーン名
                display_bone_name = (
                    f"{target_bone_name}-{(yidx + 1):02d}-{(xidx + 1):02d}"
                )
                ...
                if yidx == max_y_cnt:
                    if (
                        ...
                        # 末端は親ボーンがいたら登録
                        bone = target_bones[bone_name]
                        parent_bone = model.bones[
                            target_bone_indexes[bone.parent_index]
                        ]
        ...
        for yidx in range(0, max_y_cnt - 1):
            for xidx in range(0, max_x_idx + 1):
                ...
                # 表示先ボーン名
                display_bone_name = (
                    f"{target_bone_name}-{(yidx + 2):02d}-{(xidx + 1):02d}"
                )

                if bone_name in model.bones and display_bone_name in model.bones:
                    # 表示先ボーンを定義
                    model.bones[bone_name].tail_index = model.bones[
                        display_bone_name
                    ].index
            ...
            for xidx1, xidx2 in zip(
                ...
                if (
                    ...
                    vertices_range = vertices_map[
                        ...
                        + 1,
                        np.min(
                            [left_top_bone_map["xidx"], left_bottom_bone_map["xidx"]]
                        ) :,
                    ]

                    # 右端は最初のボーンで置き換える
                    right_top_bone_name = (
                        f"{target_bone_name}-{(yidx1 + 1):02d}-{(1):02d}"
                    )
                    right_bottom_bone_name = (
                ...
                # 剛体登録 ---------------------
                for rigidbody_target_bone_name, rigidbody_target_yidx in (
                    [(left_top_bone_name, yidx1), (left_bottom_bone_name, yidx2)]
                    if yidx2 == max_y_cnt - 1
                    else [(left_top_bone_name, yidx1)]
                ):
                    # 剛体処理対象ボーン名リスト
                    # 剛体設定
                    ...
                    # 減衰：根元から末端の線形補間
                    # 反発・摩擦：根元一定
                    mass = rigidbody_param_to.mass * (
                        (max_y_cnt - rigidbody_target_yidx + 1) ** 2
                    )
                    linear_damping = rigidbody_param_from.linear_damping + (
                        ...
                        * (rigidbody_target_yidx / max_y_cnt)
                    )
                    ...
                    horizonal_mean_length = np.mean(
                        [top_horizonal_length, bottom_horizonal_length]
                    )
                    vertical_mean_length = np.mean(
                        [left_vertical_length, right_vertical_length]
                    )
                    shape_size = MVector3D(
                        max(0.25, horizonal_mean_length * 0.5),
                        max(0.25, vertical_mean_length * 0.5),
                        0.2,
                    )

                    shape_position = model.bones[
                        rigidbody_target_bone_name
                    ].position - MVector3D(0, -0.1, 0)
                    ...
                    shape_rotation_qq = MQuaternion.rotationTo(
                        MVector3D(0, 0, -1),
                        MVector3D(shape_axis.x(), 0, shape_axis.z()),
                    )
                    # 正面の傾き
                    if rigidbody_target_yidx == 0 and xidx1 == 0:
                        shape_slope = MQuaternion.rotationTo(
                            MVector3D(0, -1, 0),
                            MVector3D(0, shape_axis.y(), shape_axis.z()),
                        )
        ...
        for yidx1, yidx2 in zip(
            list(range(0, max_y_cnt - 1)), list(range(1, max_y_cnt))
        ):
            for xidx1, xidx2 in zip(
                list(range(0, max_x_idx + 1)), list(range(1, max_x_idx + 2))
            ):
                ...
                joint_vec = model.bones[left_top_bone_name].position - (
                    (
                        model.bones[left_top_bone_name].position
                        - model.bones[left_bottom_bone_name].position
                    )
        ...
        for yidx1, yidx2 in zip(
            ...
            for xidx1, xidx2 in zip(
                ...
                if yidx2 == max_y_cnt - 1:
                    # 末端は追加で登録する
                    end_joint_name = (
                        f"→|{left_bottom_bone_name}|{right_bottom_bone_name}"
                    )
                    end_joint_vec = model.bones[left_bottom_bone_name].position - (
                        (
                            model.bones[right_bottom_bone_name].position
                            - model.bones[left_bottom_bone_name].position
                        )
                        / 2
                    )
                    end_joint_radians = model.rigidbodies[
                        left_bottom_bone_name
                    ].shape_rotation
                    end_joint = Joint(
                        end_joint_name,
                        ...
                        0,
                        model.rigidbodies[left_bottom_bone_name].index,
                        model.rigidbodies[right_bottom_bone_name].index,
                        end_joint_vec,
                        end_joint_radians,
                        MVector3D(),
                        MVector3D(),
                        MVector3D(
                            math.radians(-60), math.radians(-60), math.radians(-60)
                        ),
                        MVector3D(math.radians(60), math.radians(60), math.radians(60)),
                        MVector3D(),
                        MVector3D(),
        ...
        for yidx1, yidx2 in zip(
            ...
            for xidx1, xidx2 in zip(
                ...
                joint = Joint(
                    ...
                    MVector3D(),
                    MVector3D(
                        math.radians(-120), math.radians(-120), math.radians(-120)
                    ),
        ...
        for yidx1, yidx2 in zip(
            ...
            for xidx1, xidx2 in zip(
                ...
                joint_vec = model.bones[right_top_bone_name].position - (
                    (
                        model.bones[right_top_bone_name].position
                        - model.bones[left_bottom_bone_name].position
                    )
