...
def exec():
    ...
    model = PmxReader(
        "D:\\MMD\\Blender\\Laundry\\Laundry04.pmx", is_check=False, is_sizing=False
    ).read_data()
    ...
    model.display_slots[display_pole_name] = DisplaySlot(
        display_pole_name, display_pole_name, 0, 0
    )
    ...
    bone_xs = np.append(
        np.arange(min_vec.x(), max_vec.x(), (size.x() / max_cnt)), max_vec.x()
    )
    bone_ys = np.append(
        np.arange(max_vec.y(), min_vec.y(), -(size.y() / max_cnt)), min_vec.y()
    )

    r_size = MVector3D(
        abs(bone_xs[1] - bone_xs[0]),
        abs(bone_ys[1] - bone_ys[0]),
        abs(max_vec.z() - min_vec.z()),
    )
    ...
    for xidx in range(max_cnt + 1):
        ...
        for zidx, z_direction in enumerate([min_vec.z(), max_vec.z()]):
            for yi, yidx in enumerate(range(max_cnt + 1)):
                ...
                for k, vs in model.vertices.items():
                    for v in vs:
                        distance = v.position.distanceToPoint(
                            MVector3D(x, y, z_direction)
                        )
                ...
                parent_index = (
                    pole_bone.index if yi == 1 else len(list(model.bones.keys())) - 1
                )

                bone = Bone(
                    bone_name,
                    bone_name,
                    MVector3D(x, y, min_distance_z),
                    parent_index,
                    0,
                    0x0000 | 0x0002 | 0x0008 | 0x0010,
                )
                ...
                # 表示枠
                model.display_slots[display_name].references.append(
                    model.bones[bone.name].index
                )
    ...
    rigidbody_param_from = RigidBodyParam(
        rigidbody_param_to.mass * ((max_cnt + 1) ** 2), 0.9, 0.9, 0, 0.5
    )
    ...
    for xidx in range(max_cnt + 1):
        ...
        center_pos = MVector3D(np.mean(target_poses, axis=0)) + MVector3D(
            -r_size.x() / 2, 0, 0
        )
        ...
        mode = 0
        rigidbody = RigidBody(
            model.bones[pole_bone_name].name,
            model.bones[pole_bone_name].english_name,
            model.bones[pole_bone_name].index,
            14,
            rigidbody_no_collisions,
            0,
            MVector3D(r_size.x() / 2, r_size.y() / 2, 0.1),
            center_pos,
            MVector3D(),
            rigidbody_param_from.mass,
    ...
    for xidx in range(max_cnt + 1):
        for zidx, z_direction in enumerate([min_vec.z(), max_vec.z()]):
            for yi, yidx in enumerate(range(max_cnt + 1)):
                bone_name = (
                    f"布-{(xidx):02d}-{(zidx):02d}-{(yidx):02d}"
                    if yi > 0
                    else f"布-{(xidx):02d}-00-00"
                )
                vertical_joint_name = f"布-{(xidx):02d}-{(zidx):02d}-{(yidx + 1):02d}"

                if (
                    vertical_joint_name in model.rigidbodies
                    and bone_name in model.rigidbodies
                ):
                    joint = Joint(
                        vertical_joint_name,
                        vertical_joint_name,
                        0,
                        model.rigidbodies[bone_name].index,
                        model.rigidbodies[vertical_joint_name].index,
                        model.bones[vertical_joint_name].position
                        + MVector3D(0, r_size.y() / 2, 0),
                        MVector3D(),
                        MVector3D(),
                        MVector3D(),
                        MVector3D(
                            math.radians(-15), math.radians(-2), math.radians(-25)
                        ),
                        MVector3D(math.radians(135), math.radians(2), math.radians(25)),
                        MVector3D(),
                        MVector3D(),
                    )
    ...
    for xidx in range(max_cnt + 1):
        for zidx, z_direction in enumerate([min_vec.z(), max_vec.z()]):
            for yi, yidx in enumerate(range(max_cnt + 1)):
                ...
                if (
                    right_joint_name in model.rigidbodies
                    and bone_name in model.rigidbodies
                ):
                    joint = Joint(
                        f"{bone_name}_横",
                        ...
                        0,
                        model.rigidbodies[bone_name].index,
                        model.rigidbodies[right_joint_name].index,
                        model.bones[bone_name].position
                        + MVector3D(r_size.x() / 2, 0, 0),
                        MVector3D(),
                        MVector3D(),
                        MVector3D(),
                        MVector3D(math.radians(1), math.radians(1), math.radians(1)),
                        MVector3D(math.radians(0), math.radians(0), math.radians(0)),
                        MVector3D(),
                        MVector3D(),
                    )
    ...
    for k, vs in model.vertices.items():
        for v in vs:
            ...
            r_min_vec = MVector3D(
                list(target_xs.keys())[0],
                list(target_ys.keys())[-1],
                list(target_zs.keys())[0],
            )
            r_max_vec = MVector3D(
                list(target_xs.keys())[-1],
                list(target_ys.keys())[0],
                list(target_zs.keys())[-1],
            )

            left_top_weight = max(
                0,
                ((r_size.x() - (v.position.x() - r_min_vec.x())) / r_size.x())
                * ((v.position.y() - r_min_vec.y()) / r_size.y()),
            )
            left_bottom_weight = max(
                0,
                ((r_size.x() - (v.position.x() - r_min_vec.x())) / r_size.x())
                * (r_size.y() - (v.position.y() - r_min_vec.y()))
                / r_size.y(),
            )
            right_bottom_weight = max(
                0,
                ((v.position.x() - r_min_vec.x()) / r_size.x())
                * (r_size.y() - (v.position.y() - r_min_vec.y()))
                / r_size.y(),
            )
            ...
            right_bottom_bone = model.bones[
                f"布-{(max_cnt):02d}-{(0):02d}-{(max_cnt):02d}"
            ]
            ...
            for bone in model.bones.values():
                ...
                if (
                    bone.position.x() <= v.position.x()
                    and bone.position.y() >= v.position.y()
                    and distance < left_top_distance
                ):
                    left_top_bone = bone
                ...
                if (
                    bone.position.x() >= v.position.x()
                    and bone.position.y() >= v.position.y()
                    and distance < right_top_distance
                ):
                    right_top_bone = bone
                ...
                if (
                    bone.position.x() <= v.position.x()
                    and bone.position.y() <= v.position.y()
                    and distance < left_bottom_distance
                ):
                ...
                if (
                    bone.position.x() >= v.position.x()
                    and bone.position.y() <= v.position.y()
                    and distance < right_bottom_distance
                ):
            ...
            weight_names = np.array(
                [
                    left_top_bone.name,
                    right_top_bone.name,
                    left_bottom_bone.name,
                    right_bottom_bone.name,
                ]
            )
            ...
            if np.count_nonzero(weight_values) == 1:
                ...
                v.deform = Bdef4(
                    left_top_bone.index,
                    right_top_bone.index,
                    left_bottom_bone.index,
                    right_bottom_bone.index,
                    weight_values[0],
                    weight_values[1],
                    weight_values[2],
                    weight_values[3],
