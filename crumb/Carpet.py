...
def exec():
    ...
    bone_zs = np.append(
        np.arange(max_vec.z(), min_vec.z(), -(size.z() / max_cnt)), min_vec.z()
    )
    ...
    for zidx in range(max_cnt + 1):
        for xidx in range(max_cnt + 1):
            ...
            # 剛体
            bone_rigidbody = RigidBody(
                f"{bone.name}_追従",
                f"{bone.name}_追従",
                bone.index,
                14,
                bone_rigidbody_no_collisions,
                1,
                MVector3D(abs(r_size.x() / 2), 0.5, r_size.z() / 2),
                center_pos,
                MVector3D(),
                0.5,
                0.5,
        ...
        for xidx in range(max_cnt + 1):
            ...
            if (
                ...
                joint = Joint(
                    ...
                    model.bones[bone_name].position - MVector3D(0, 0, r_size.z() / 2),
                    MVector3D(),
                    MVector3D(),
                    MVector3D(),
                    MVector3D(
                        math.radians(-(max_cnt * 5)), math.radians(0), math.radians(0)
                    ),
                    MVector3D(
                        math.radians((max_cnt * 3)), math.radians(0), math.radians(0)
                    ),
        ...
        for xidx in range(max_cnt + 1):
            ...
            if (
                ...
                joint = Joint(
                    ...
                    0,
                    model.rigidbodies[bone_name].index,
                    model.rigidbodies[right_bottom_joint_name].index,
                    model.bones[bone_name].position + MVector3D(r_size.x() / 2, 0, 0),
                    MVector3D(),
                    MVector3D(),
                    MVector3D(),
                    MVector3D(
    ...
    for xidx, (k, vs) in enumerate(model.vertices.items()):
        for zidx, v in enumerate(vs):
            ...
            weight_names = np.array(
                [left_top_name, right_top_name, left_bottom_name, right_bottom_name]
            )
    ...
    bone = Bone(
        ...
        bone_name,
        MVector3D(),
        model.bones[f"{model_prefix}{center_cnt:02d}-{center_cnt:02d}"].index,
        0,
        0x0000 | 0x0002 | 0x0004 | 0x0008 | 0x0010 | 0x1000,
    )
    bone.index = len(list(model.bones.keys()))
    model.bones[bone.name] = bone
    ...
    result_dir = (
        "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Accessory\\家具\\空飛ぶ絨毯 miu"
    )
