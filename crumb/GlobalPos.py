...
def calc_global_pos():
    model = PmxReader(
        "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Model\\VOCALOID\\初音ミク\\ISAO式ミク\\I_ミクv4\\Miku_V4.pmx",
        is_check=False,
    ).read_data()
    ...
    motion = VmdReader(
        "D:\\MMD\\MikuMikuDance_v926x64\\UserFile\\Motion\\ダンス_1人\\エゴロック 粉ふきスティック\\egorock_miku.vmd"
    ).read_data()
    ...
    target_links = [
        model.create_link_2_top_one("左人指先実体", "左手首"),
        model.create_link_2_top_one("右人指先実体", "右手首"),
    ]

    for fno in range(0, 10, 10):
        for links in target_links:
            result_3ds, result_mats = MServiceUtils.calc_global_pos(
                model,
                links,
                motion,
                fno,
            ...
            logger.debug_info(
                "%s [%s] -----------------------------", fno, links.last_name()
            )
            for (link_name, result_pos), result_mat in zip(
                result_3ds.items(), result_mats.values()
            ):
    ...
    result_dir = (
        "D:\\MMD\\MikuMikuDance_v926x64\\Work\\x003_VMDサイジング\\ver5.02\\test"
    )
    ...
    for bidx, b in enumerate(result_pmx.bones.values()):
        try:
            if bidx > 0:
                b.tail_index = result_pmx.bones[
                    model.bone_indexes[model.bones[b.name].parent_index]
                ].index
                b.flag |= 0x0001
        except:
            pass
    ...
    pmx_writer.write(
        result_pmx, f"{result_dir}\\model_{datetime.now():%Y%m%d_%H%M%S}.pmx"
    )
...
def create_bone(org_model: PmxModel, model: PmxModel, mname: str):
    ...
    color_list = [
        ...
        MVector3D(51, 255, 51),
        MVector3D(51, 51, 255),
        MVector3D(255, 127, 51),
        MVector3D(255, 51, 127),
        MVector3D(127, 255, 51),
        MVector3D(255, 127, 51),
        MVector3D(51, 127, 255),
        MVector3D(51, 255, 127),
        MVector3D(127, 127, 51),
    ]
    ...
    # 材質
    model.materials[bone.name] = Material(
        bone.name,
        bone.name,
        color,
        1,
    ...
    for vidx, vpos in enumerate(
        [
            ...
            MVector3D(
                local_x_qq * MVector3D(-0.000000014570465, 0.6666665, -0.16666665)
            ),
            MVector3D(
                local_x_qq * MVector3D(-0.16666665, 0.6666665, -0.000000007946625)
            ),
            MVector3D(local_x_qq * MVector3D(0, 0.33333335, 0.33333335)),
            MVector3D(
                local_x_qq * MVector3D(0.33333335, 0.3333333, -0.0000000046363535)
            ),
            MVector3D(
                local_x_qq * MVector3D(-0.000000029140925, 0.3333333, -0.3333333)
            ),
            ...
            MVector3D(
                local_x_qq * MVector3D(-0.5, 0.00000002980232, 0.000000035764765)
            ),
            MVector3D(local_x_qq * MVector3D(0, 0.00000005960465, 0.5)),
            MVector3D(local_x_qq * MVector3D(0.5, 0.00000002980232, 0.00000000794663)),
            MVector3D(local_x_qq * MVector3D(-0.00000004371139, 0, -0.49999995)),
